...
'    Dim sRsPtn      As String       '比抵抗ﾃﾞｰﾀ取得ﾊﾟﾀｰﾝ　04/02/12 ooba
    ...
    Dim sMSMPFLG  As String                      '中間抜試フラグ
...
'            End If
            'C評価
            If getTBCMJ023C(CRYNUM, recXSDCS(i), recX006(i)) = FUNCTION_RETURN_FAILURE Then
                errmsg = "J023C-:" & XlSmpPos(i)
                GoTo proc_exit
            End If
            
            'CJ評価
            If getTBCMJ023CJ(CRYNUM, recXSDCS(i), recX006(i)) = FUNCTION_RETURN_FAILURE Then
                errmsg = "J023CJ-:" & XlSmpPos(i)
                GoTo proc_exit
            End If
            
            'CJLT評価
            If getTBCMJ023CJLT(CRYNUM, recXSDCS(i), recX006(i)) = FUNCTION_RETURN_FAILURE Then
                errmsg = "J023CJLT-:" & XlSmpPos(i)
                GoTo proc_exit
            End If
            
            'CJ2評価
            If getTBCMJ023CJ2(CRYNUM, recXSDCS(i), recX006(i)) = FUNCTION_RETURN_FAILURE Then
                errmsg = "J023CJ2-:" & XlSmpPos(i)
                GoTo proc_exit
            End If
'Cng End 2011/11/8 Y.Hitomi
...
'>>>>> 2011/06/00 SETsw)Marushita 中間抜試サンプル送信対応
    ...
    If getTBCME036(HIN, iMCUTUNIT, sMSMPFLG) = FUNCTION_RETURN_FAILURE Then
        ...
        GoTo proc_exit
    End If
'Cng Start 2011/10/19 Y.Hitomi
    If recCnt > 0 Then
'    If recCnt > 0 And sMSMPFLG = "1" Then
'Cng End 2011/10/19 Y.Hitomi
        For i = 1 To recCnt
            '-------------------- TBCMX007固定情報データ設定 ----------------------------------------
...
'            If (wHSXRHWYS = "H") And (.Fields("SXLRS_RRG").Value = -1) And CRes.GuaranteeRes.cObj <> "1" Then
'                GoTo proc_exit
'            End If
'            'Cng End 2011/10/12 Y.Hitomi
'Cng End 2011/10/25 Y.Hitomi
...
''    Const keisu As Double = 1        'BMDべき乗数変更対応　2003/05/19 osawa
    ...
    '' 2006/09/25 SMP)kondoh Add -e-
    Const keisu9 As Double = 10000 'Add 2012/07/23 Y.Hitomi
    ...
    If (recXSDCW("WFINDB" & j & "CW").Value <> "0") And (recXSDCW("WFRESB" & j & "CW").Value <> "0") Then
        ...
        ElseIf bm.GuaranteeBmd.cMeth = "8" And bm.GuaranteeBmd.cCount = "4" And bm.GuaranteeBmd.cPos = "8" Then
            keisu = keisu8
        'Add Start 2012/07/23 Y.Hitomi
        ElseIf bm.GuaranteeBmd.cMeth = "P" Then
            keisu = keisu9
        'Add End 2012/07/23 Y.Hitomi
        Else
            keisu = -1
        End If
        '' 2006/09/25 SMP)kondoh Add -e-
...
Private Function getTBCMY022EPBMD(recXSDCW As c_cmzcrec, j As Integer, _
    ...
    (recXSDCW("EPRESB" & j & "CW").Value <> "0") Then
        ...
        ElseIf ebmd.GuaranteeBmd.cMeth = "P" Then
            keisu = keisu9
...
'''            If i = 1 Then sTBkbn = "T" Else sTBkbn = "B"
        ...
        If rs.RecordCount > 0 Then
        'Cng Start 2012/01/17 Y.Hitomi
            'TOP側実績ﾃﾞｰﾀ
            If sTBkbn = "T" Then
                mesdata(1) = rs("MESDATA5")
                mesdata(2) = rs("MESDATA4")
                mesdata(3) = rs("MESDATA3")
                mesdata(4) = rs("MESDATA2")
                mesdata(5) = rs("MESDATA1")
            'BOT側実績ﾃﾞｰﾀ
            ElseIf sTBkbn = "B" Then
                mesdata(6) = rs("MESDATA5")
                mesdata(7) = rs("MESDATA4")
                mesdata(8) = rs("MESDATA3")
                mesdata(9) = rs("MESDATA2")
                mesdata(10) = rs("MESDATA1")
            End If
'            'TOP側実績ﾃﾞｰﾀ
'            If sTBkbn = "T" Then
...
Private Function GetStandardPosRes(ByVal CRYNUM As String, _
    ...
    Dim wgtTop      As Double               '偏析計算用パラメータ(Top取量)->(肩重量)※
    Dim wgtTopCut   As Double               '偏析計算用パラメータ(肩重量)->(Top取量)※
    ...
    d.TOPWEIGHT = wgtTop + wgtTopCut    'トップ重量(Top取量＋肩重量)->(肩重量＋Top取量)※
    ...
    If HIKIFLG = "1" Then       '通常引上の場合
        'Top取量/(断面積*0.00233)
        '※Top取量がTop重量(肩重量)になっていたため変更　2011/11/11 Marushita
        d.HOSEICHO = wgtTopCut / (d.DUNMENSEKI * HIJU_SILICONE)
    ...
    ElseIf HIKIFLG = "2" Then   'B、C結晶(残量引)の場合
        ...
        '※Top取量がTop重量(肩重量)になっていたため変更　2011/11/11 Marushita
        d.HOSEICHO = dMaeBatLen + wgtTopCut / (d.DUNMENSEKI * HIJU_SILICONE)
...
Private Function getTBCME036(ByRef HIN As tFullHinban, _
                                        ByRef iUnit As Integer, ByRef sFlg As String) As FUNCTION_RETURN
'Private Function getTBCME036(ByRef HIN As tFullHinban, _
...
proc_err:
    ...
    Resume proc_exit
End Function
'概要      :CLESTA評価実績(TBCMJ023) Cデータ取得設定
'ﾊﾟﾗﾒｰﾀ　　:変数名          , IO , 型               , 説明
'          :CRYNUM          , I  ,String            , 結晶番号
'          :recXSDCS        , I  ,c_cmzcrec         , XSDCS構造体   (新ｻﾝﾌﾟﾙ管理(ﾌﾞﾛｯｸ))
'          :recX006         , O  ,c_cmzcrec         , TBCMX006構造体(Cu-Deco構造体)
'      　　:戻り値          , O  ,FUNCTION_RETURN　 , 成否
'説明      :CLESTA評価実績(TBCMJ023)からCﾃﾞｰﾀを取得し、Cu-Deco構造体にｾｯﾄする
'履歴      :2011/11/8  Y.Hitomi
Private Function getTBCMJ023C(CRYNUM As String, recXSDCS As c_cmzcrec, recX006 As c_cmzcrec) As FUNCTION_RETURN
    Dim rs          As OraDynaset
    Dim sql         As String
    ...
    getTBCMJ023C = FUNCTION_RETURN_FAILURE
    ...
    With recX006
        ...
        '-------------------- TBCMJ023の読み込み(CJ) ----------------------------------------
        'Add Start 2012/02/14 Y.Hitomi
        If IsNull(recXSDCS("CRYSMPLIDCCS").Value) = True Then
            getTBCMJ023C = FUNCTION_RETURN_SUCCESS
            GoTo proc_exit
        End If
        'Add End 2012/02/14 Y.Hitomi

        sql = "select * from TBCMJ023 "
        sql = sql & "where CRYNUM = '" & CRYNUM & "' and "
        sql = sql & "      SMPLNO = " & recXSDCS("CRYSMPLIDCCS").Value
        sql = sql & "order by TRANCNT desc"
        sql = "select * from (" & sql & ") where rownum = 1"
        Debug.Print (sql)
        Set rs = OraDB.DBCreateDynaset(sql, ORADYN_NO_BLANKSTRIP)
        If rs.RecordCount = 0 Then
            Set rs = Nothing
            getTBCMJ023C = FUNCTION_RETURN_SUCCESS                     '2011/02/28 tkimura
            GoTo proc_exit
        End If
...
'概要      :CLESTA評価実績(TBCMJ023) CJデータ取得設定
'ﾊﾟﾗﾒｰﾀ　　:変数名          , IO , 型               , 説明
'          :CRYNUM          , I  ,String            , 結晶番号
'          :recXSDCS        , I  ,c_cmzcrec         , XSDCS構造体   (新ｻﾝﾌﾟﾙ管理(ﾌﾞﾛｯｸ))
'          :recX006         , O  ,c_cmzcrec         , TBCMX006構造体(Cu-Deco構造体)
'      　　:戻り値          , O  ,FUNCTION_RETURN　 , 成否
'説明      :CLESTA評価実績(TBCMJ023)からCJﾃﾞｰﾀを取得し、Cu-Deco構造体にｾｯﾄする
'履歴      :2011/11/8  Y.Hitomi
Private Function getTBCMJ023CJ(CRYNUM As String, recXSDCS As c_cmzcrec, recX006 As c_cmzcrec) As FUNCTION_RETURN
    Dim rs          As OraDynaset
    Dim sql         As String
    ...
    getTBCMJ023CJ = FUNCTION_RETURN_FAILURE
    ...
    With recX006
        ...
        '-------------------- TBCMJ023の読み込み(CJ) ----------------------------------------
        'Add Start 2012/02/14 Y.Hitomi
        If IsNull(recXSDCS("CRYSMPLIDCJCS").Value) = True Then
            getTBCMJ023CJ = FUNCTION_RETURN_SUCCESS
            GoTo proc_exit
        End If
        'Add End 2012/02/14 Y.Hitomi
        ...
        sql = "select * from TBCMJ023 "
        sql = sql & "where CRYNUM = '" & CRYNUM & "' and "
        sql = sql & "      SMPLNO = " & recXSDCS("CRYSMPLIDCJCS").Value
        sql = sql & "order by TRANCNT desc"
        sql = "select * from (" & sql & ") where rownum = 1"
        Debug.Print (sql)
        Set rs = OraDB.DBCreateDynaset(sql, ORADYN_NO_BLANKSTRIP)
...
proc_err:
    ...
    Resume proc_exit
End Function
'概要      :CLESTA評価実績(TBCMJ023) CJLTデータ取得設定
'ﾊﾟﾗﾒｰﾀ　　:変数名          , IO , 型               , 説明
'          :CRYNUM          , I  ,String            , 結晶番号
'          :recXSDCS        , I  ,c_cmzcrec         , XSDCS構造体   (新ｻﾝﾌﾟﾙ管理(ﾌﾞﾛｯｸ))
'          :recX006         , O  ,c_cmzcrec         , TBCMX006構造体(Cu-Deco構造体)
'      　　:戻り値          , O  ,FUNCTION_RETURN　 , 成否
'説明      :CLESTA評価実績(TBCMJ023)からCJLTﾃﾞｰﾀを取得し、Cu-Deco構造体にｾｯﾄする
'履歴      :2011/11/8  Y.Hitomi
Private Function getTBCMJ023CJLT(CRYNUM As String, recXSDCS As c_cmzcrec, recX006 As c_cmzcrec) As FUNCTION_RETURN
    Dim rs          As OraDynaset
    Dim sql         As String
    ...
    getTBCMJ023CJLT = FUNCTION_RETURN_FAILURE
    ...
    With recX006
        ...
        '-------------------- TBCMJ023の読み込み(CJ) ----------------------------------------
        'Add Start 2012/02/14 Y.Hitomi
        If IsNull(recXSDCS("CRYSMPLIDCJLTCS").Value) = True Then
            getTBCMJ023CJLT = FUNCTION_RETURN_SUCCESS
            GoTo proc_exit
        End If
        'Add End 2012/02/14 Y.Hitomi
        ...
        sql = "select * from TBCMJ023 "
        sql = sql & "where CRYNUM = '" & CRYNUM & "' and "
        sql = sql & "      SMPLNO = " & recXSDCS("CRYSMPLIDCJLTCS").Value
        sql = sql & "order by TRANCNT desc"
        sql = "select * from (" & sql & ") where rownum = 1"
        Debug.Print (sql)
        Set rs = OraDB.DBCreateDynaset(sql, ORADYN_NO_BLANKSTRIP)
...
proc_err:
    ...
    Resume proc_exit
End Function
'概要      :CLESTA評価実績(TBCMJ023) CJ2データ取得設定
'ﾊﾟﾗﾒｰﾀ　　:変数名          , IO , 型               , 説明
'          :CRYNUM          , I  ,String            , 結晶番号
'          :recXSDCS        , I  ,c_cmzcrec         , XSDCS構造体   (新ｻﾝﾌﾟﾙ管理(ﾌﾞﾛｯｸ))
'          :recX006         , O  ,c_cmzcrec         , TBCMX006構造体(Cu-Deco構造体)
'      　　:戻り値          , O  ,FUNCTION_RETURN　 , 成否
'説明      :CLESTA評価実績(TBCMJ023)からCJ2ﾃﾞｰﾀを取得し、Cu-Deco構造体にｾｯﾄする
'履歴      :2011/11/8  Y.Hitomi
Private Function getTBCMJ023CJ2(CRYNUM As String, recXSDCS As c_cmzcrec, recX006 As c_cmzcrec) As FUNCTION_RETURN
    Dim rs          As OraDynaset
    Dim sql         As String
    ...
    getTBCMJ023CJ2 = FUNCTION_RETURN_FAILURE
    ...
    With recX006
        ...
        '-------------------- TBCMJ023の読み込み(CJ) ----------------------------------------
        'Add Start 2012/02/14 Y.Hitomi
        If IsNull(recXSDCS("CRYSMPLIDCJ2CS").Value) = True Then
            getTBCMJ023CJ2 = FUNCTION_RETURN_SUCCESS
            GoTo proc_exit
        End If
        'Add End 2012/02/14 Y.Hitomi
        ...
        sql = "select * from TBCMJ023 "
        sql = sql & "where CRYNUM = '" & CRYNUM & "' and "
        sql = sql & "      SMPLNO = " & recXSDCS("CRYSMPLIDCJ2CS").Value
        sql = sql & "order by TRANCNT desc"
        sql = "select * from (" & sql & ") where rownum = 1"
        Debug.Print (sql)
        Set rs = OraDB.DBCreateDynaset(sql, ORADYN_NO_BLANKSTRIP)
