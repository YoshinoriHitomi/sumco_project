...
'Add Start 2011/04/12 SMPK Miyata
        sql = "select"
        sql = sql & " nvl(MSMPFLGWFR, '0') as MSMPFLGWFR, "         ' 中間抜試フラグ/WF/比抵抗　[Rs]
        sql = sql & " nvl(MSMPFLGWFO, '0') as MSMPFLGWFO, "         ' 中間抜試フラグ/WF/酸素　[Oi]
        sql = sql & " nvl(MSMPFLGWFOF, '0') as MSMPFLGWFOF, "       ' 中間抜試フラグ/WF/OSF　[O1-O3]
        sql = sql & " nvl(MSMPFLGWFBM, '0') as MSMPFLGWFBM, "       ' 中間抜試フラグ/WF/BMD　[B1-B3]
        sql = sql & " nvl(MSMPFLGWFGD, '0') as MSMPFLGWFGD, "       ' 中間抜試フラグ/WF/GD　[GD]
        sql = sql & " nvl(MSMPFLGWFDOI, '0') as MSMPFLGWFDOI, "     ' 中間抜試フラグ/WF/Δoi（酸素析出）[DOI1-DOI3]
        sql = sql & " nvl(MSMPFLGWFAOI, '0') as MSMPFLGWFAOI, "     ' 中間抜試フラグ/WF/AOI（残存酸素）[AO]
        sql = sql & " nvl(MSMPFLGWFDS, '0') as MSMPFLGWFDS, "       ' 中間抜試フラグ/WF/DSOD  [DS]
        sql = sql & " nvl(MSMPFLGWFDZ, '0') as MSMPFLGWFDZ, "       ' 中間抜試フラグ/WF/DZ　　[DZ]
        sql = sql & " nvl(MSMPFLGWFOT1, '0') as MSMPFLGWFOT1, "     ' 中間抜試フラグ/WF/その他1  [OT1]
        sql = sql & " nvl(MSMPWFOT1MAI, '0') as MSMPWFOT1MAI, "     ' 中間抜試フラグ/WF/その他1　枚数
        sql = sql & " nvl(to_char(MSMPWFOT1TIM, 'YYYY/MM/DD'), ' ') AS MSMPWFOT1TIM, "    ' 中間抜試フラグ/WF/その他1　期限
        sql = sql & " nvl(decode(SIGN(MSMPWFOT1TIM-SYSDATE), 1, '1', 0, '1', -1, '0'), '0' ) AS MSMPWFOT1TIM_FLG, "    ' 中間抜試フラグ/WF/その他1　期限フラグ
        sql = sql & " nvl(MSMPFLGEPOF, '0') as MSMPFLGEPOF, "       ' 中間抜試フラグ/EP/OSF [O1E-O3E]
        sql = sql & " nvl(MSMPFLGEPBM, '0') as MSMPFLGEPBM, "       ' 中間抜試フラグ/EP/BMD [B1E-B3E]
        sql = sql & " nvl(MSMPFLGEPOT2, '0') as MSMPFLGEPOT2, "     ' 中間抜試フラグ/EP/その他2  [OT2]
        sql = sql & " nvl(MSMPWFOT2MAI, '0') as MSMPWFOT2MAI, "     ' 中間抜試フラグ/EP/その他2　枚数
        sql = sql & " nvl(to_char(MSMPWFOT2TIM, 'YYYY/MM/DD'), ' ') AS MSMPWFOT2TIM, "    ' 中間抜試フラグ/WF/その他2　期限
        sql = sql & " nvl(decode(SIGN(MSMPWFOT2TIM-SYSDATE), 1, '1', 0, '1', -1, '0'), '0' ) AS MSMPWFOT2TIM_FLG, "    ' 中間抜試フラグ/WF/その他2　期限フラグ
        sql = sql & " nvl(MSMPTANI, 0) as MSMPTANI, "               ' 中間抜試単位
        sql = sql & " nvl(MSMPFLG, '0') as MSMPFLG, "               ' 中間抜試フラグ
        sql = sql & " nvl(MSMPTANIMAI, '0') as MSMPTANIMAI, "       ' 中間抜試単位(枚数)
        sql = sql & " nvl(MSMPCONSTMAI, '0') as MSMPCONSTMAI "      ' 中間抜試許容値(枚数)
        sql = sql & " from  TBCME036"
        sql = sql & " where HINBAN  ='" & .HIN.hinban & "'"
        sql = sql & "   and MNOREVNO= " & .HIN.mnorevno
        sql = sql & "   and FACTORY ='" & .HIN.factory & "'"
        ...
        .MSMPFLGWFR = rs("MSMPFLGWFR")          ' 中間抜試フラグ/WF/比抵抗　[Rs]
        .MSMPFLGWFO = rs("MSMPFLGWFO")          ' 中間抜試フラグ/WF/酸素　[Oi]
        .MSMPFLGWFOF = rs("MSMPFLGWFOF")        ' 中間抜試フラグ/WF/OSF　[O1-O3]
        .MSMPFLGWFBM = rs("MSMPFLGWFBM")        ' 中間抜試フラグ/WF/BMD　[B1-B3]
        .MSMPFLGWFGD = rs("MSMPFLGWFGD")        ' 中間抜試フラグ/WF/GD　[GD]
        .MSMPFLGWFDOI = rs("MSMPFLGWFDOI")      ' 中間抜試フラグ/WF/Δoi（酸素析出）[DOI1-DOI3]
        .MSMPFLGWFAOI = rs("MSMPFLGWFAOI")      ' 中間抜試フラグ/WF/AOI（残存酸素）[AO]
        .MSMPFLGWFDS = rs("MSMPFLGWFDS")        ' 中間抜試フラグ/WF/DSOD  [DS]
        .MSMPFLGWFDZ = rs("MSMPFLGWFDZ")        ' 中間抜試フラグ/WF/DZ　　[DZ]
                                                ' 中間抜試フラグ/WF/その他1  [OT1]
        '中間抜試フラグが「指示あり」で現在日時が期限以降であれば中間抜試指示ありとする
        If rs("MSMPFLGWFOT1") = "1" And rs("MSMPWFOT1TIM_FLG") = "1" Then
            .MSMPFLGWFOT1 = "1"                 ' 指示あり
        Else
            .MSMPFLGWFOT1 = "0"                 ' 指示なし
        End If
        .MSMPWFOT1MAI = rs("MSMPWFOT1MAI")      ' 中間抜試フラグ/WF/その他1　枚数
        .MSMPWFOT1TIM = rs("MSMPWFOT1TIM")      ' 中間抜試フラグ/WF/その他1　期限
        .MSMPFLGEPOF = rs("MSMPFLGEPOF")        ' 中間抜試フラグ/EP/OSF [O1E-O3E]
        .MSMPFLGEPBM = rs("MSMPFLGEPBM")        ' 中間抜試フラグ/EP/BMD [B1E-B3E]
                                                ' 中間抜試フラグ/EP/その他2  [OT2]
        '中間抜試フラグが「指示あり」で現在日時が期限以降であれば中間抜試指示ありとする
        If rs("MSMPFLGEPOT2") = "1" And rs("MSMPWFOT2TIM_FLG") = "1" Then
            .MSMPFLGEPOT2 = "1"                 ' 指示あり
        Else
            .MSMPFLGEPOT2 = "0"                 ' 指示なし
        End If

        .MSMPWFOT2MAI = rs("MSMPWFOT2MAI")      ' 中間抜試フラグ/EP/その他2　枚数
        .MSMPWFOT2TIM = rs("MSMPWFOT2TIM")      ' 中間抜試フラグ/WF/その他2　期限
        .MSMPTANI = rs("MSMPTANI")              ' 中間抜試単位
        .MSMPFLG = rs("MSMPFLG")                ' 中間抜試フラグ
        .MSMPTANIMAI = rs("MSMPTANIMAI")        ' 中間抜試単位(枚数)
        .MSMPCONSTMAI = rs("MSMPCONSTMAI")      ' 中間抜試許容値(枚数)

        rs.Close
        
'Add End   2011/04/12 SMPK Miyata
    End With
...
'Add Start 2011/04/12 SMPK Miyata
'---------------------------------------------------------------------------
'概要      :WFﾏｯﾌﾟ管理ﾃｰﾌﾞﾙ（TBCMY011）からﾃﾞｰﾀを取得
'---------------------------------------------------------------------------
'ﾊﾟﾗﾒｰﾀ    :変数名      ,IO     ,型                     ,説明
'          :records()   ,O      ,typeWFmap              ,抽出レコード
'          :sqlWhere    ,I      ,String                 ,抽出条件(SQLのWhere節:省略可能)
'          :sqlOrder    ,I      ,String                 ,抽出順序(SQLのOrder by節:省略可能)
'          :戻り値      ,O      ,FUNCTION_RETURN        ,抽出の成否
'---------------------------------------------------------------------------
Public Function fncGetWFmap(records() As typeWFmap, sqlWhere As String, sqlOrder As String) As FUNCTION_RETURN
    ...
    fncGetWFmap = FUNCTION_RETURN_FAILURE
    ...
    ReDim records(0)
    For iDataCnt = 1 To rs.RecordCount
    
        ReDim Preserve records(iDataCnt) As typeWFmap
        With records(iDataCnt)
            .LOTID = NulltoStr(rs.Fields("LOTID"))                     'ブロックID
            .BLOCKSEQ = NulltoStr(rs.Fields("BLOCKSEQ"))               'ブロック内連番
            .INDTM = NulltoStr(rs.Fields("INDTM"))                     'ウェハーセンター入庫日時
            .BASKETID = NulltoStr(rs.Fields("BASKETID"))               'バスケットID
            .SLOTNO = NulltoStr(rs.Fields("SLOTNO"))                   'スロットNo
            .CURRWPCS = NulltoStr(rs.Fields("CURRWPCS"))               'ウェハー枚数
            .EXISTFLG = NulltoStr(rs.Fields("EXISTFLG"))               '存在フラグ
            .TOP_POS = NulltoStr(rs.Fields("TOP_POS"))                 'ブロックのTopからの 位置
            .REJCAT = NulltoStr(rs.Fields("REJCAT"))                   '欠落理由
            .TXID = NulltoStr(rs.Fields("TXID"))                       'トランザクションID
            .REGDATE = NulltoStr(rs.Fields("REGDATE"))                 '登録日付
            .SUMMITSENDFLG = NulltoStr(rs.Fields("SUMMITSENDFLAG"))    'SUMMIT送信フラグ
            .SENDFLG = NulltoStr(rs.Fields("SENDFLAG"))                '送信フラグ
            .SENDDATE = NulltoStr(rs.Fields("SENDDATE"))               '送信日付
            .WFSTA = NulltoStr(rs.Fields("WFSTA"))                     'WF状態
            .HREJCODE = NulltoStr(rs.Fields("HREJCODE"))               '不良理由コード
            .UPDPROC = NulltoStr(rs.Fields("UPDPROC"))                 '更新工程
            .UPDDATE = NulltoStr(rs.Fields("UPDDATE"))                 '更新日時
            .SXLID = NulltoStr(rs.Fields("MSXLID"))                    'SXLID
            .hinban = NulltoStr(rs.Fields("MHINBAN"))                  '品番
            .REVNUM = NulltoStr(rs.Fields("MREVNUM"))                  '製品番号改訂番号
            .factory = NulltoStr(rs.Fields("MFACTORY"))                '工場
            .opecond = NulltoStr(rs.Fields("MOPECOND"))                '操業条件
            .KANKBN = NulltoStr(rs.Fields("KANKBN"))                   '完了区分
            .SMPLEID = NulltoStr(rs.Fields("MSMPLEID"))                '抜試位置
            .NREJCODE = NulltoStr(rs.Fields("NREJCODE"))               '抜試返答理由コード
            .SMPLEFLG = NulltoStr(rs.Fields("SHAFLAG"))               'サンプルフラグ
            .RTOP_POS = NulltoStr(rs.Fields("RTOP_POS"))               '論理ブロック内位置
            .RITOP_POS = NulltoStr(rs.Fields("RITOP_POS"))             '論理結晶内位置
        End With
        rs.MoveNext
    Next iDataCnt
    rs.Close
    
    fncGetWFmap = FUNCTION_RETURN_SUCCESS
    Exit Function
...
'          :戻り値      ,O      ,FUNCTION_RETURN        ,抽出の成否
'---------------------------------------------------------------------------
Public Function fncGetWFmap_MaiCnt(records() As typeWFmap, pLOTID As String, pBLOCKSEQ As Integer, pMaiCnt As Integer) As FUNCTION_RETURN

    Dim strSQL      As String       'SQL文
    Dim rs          As OraDynaset   'RecordSet
    Dim iDataCnt    As Integer      'データカウンタ

    On Error GoTo PROC_ERR

    fncGetWFmap_MaiCnt = FUNCTION_RETURN_FAILURE

    'SQL文編集
    '該当ブロックの欠落WFを除く先頭からWF枚数位置にあるWF情報をSelectする
    strSQL = "SELECT * "
    strSQL = strSQL & " FROM ("
    strSQL = strSQL & " SELECT"
    strSQL = strSQL & "  LOTID, BLOCKSEQ, INDTM, BASKETID, SLOTNO"
    strSQL = strSQL & ", CURRWPCS, EXISTFLG, TOP_POS, REJCAT"
    strSQL = strSQL & ", TXID, REGDATE, SUMMITSENDFLAG"
    strSQL = strSQL & ", SENDFLAG, SENDDATE, WFSTA, HREJCODE, UPDPROC"
    strSQL = strSQL & ", UPDDATE, MSXLID, MHINBAN, MREVNUM, MFACTORY"
    strSQL = strSQL & ", MOPECOND, KANKBN, MSMPLEID, NREJCODE, SHAFLAG"
    strSQL = strSQL & ", RTOP_POS, RITOP_POS"
    strSQL = strSQL & ", RANK() OVER( ORDER BY LOTID, BLOCKSEQ) AS MAICNT"
    strSQL = strSQL & " FROM TBCMY011"
    strSQL = strSQL & " WHERE LOTID='" & pLOTID & "'"
    strSQL = strSQL & " AND BLOCKSEQ >= " & pBLOCKSEQ
    strSQL = strSQL & " AND EXISTFLG = 'Y' )"
    strSQL = strSQL & " WHERE MAICNT=" & pMaiCnt
    ...
    fncGetWFmap_MaiCnt = FUNCTION_RETURN_SUCCESS
    Exit Function
...
'概要    :WFマップテーブル更新(中間抜試サンプル)
'ﾊﾟﾗﾒｰﾀ  :変数名       ,IO   ,型                                    ,説明
'        :pWFmap       , O   ,typeWFmap        ,                    中間抜試のWF情報
'        :戻ﾘ値        , O   ,FUNCTION_RETURN                       ,読み込み成否
'説明    :WFマップテーブル(TBCMY011)を更新する
'履歴    :
Public Function DBDRV_UPD_Mid_WFMap(pWFmap() As typeWFmap) As FUNCTION_RETURN

    Dim sql             As String           'SQL文編集
    Dim rs              As OraDynaset       'レコードセット
    Dim i               As Integer          'カウンタ
    Dim sDbName         As String           'DB名

    'エラーハンドラの設定
    On Error GoTo PROC_ERR
    gErr.Push "s_cmbc039_SQL.bas -- Function DBDRV_UPD_Mid_WFMap"

    sDbName = "(Y011)"
    For i = 1 To UBound(pWFmap)
    
        sql = "UPDATE TBCMY011 SET"
        sql = sql & " MSMPLEID= '" & pWFmap(i).SMPLEID & "'"        ' 抜試位置
        sql = sql & ",SHAFLAG = '" & pWFmap(i).SMPLEFLG & "'"       ' サンプルフラグ"
        sql = sql & ",WFSTA   = '" & pWFmap(i).WFSTA & "'"          ' WF状態サンプル
        sql = sql & ",UPDPROC = 'CW740'"                            ' 更新工程
        sql = sql & ",UPDDATE = sysdate"                            ' 更新日時
        sql = sql & " WHERE LOTID   = '" & pWFmap(i).LOTID & "'"    ' ブロックID"
        sql = sql & "   AND BLOCKSEQ=  " & pWFmap(i).BLOCKSEQ       ' ブロック内連番"
    ...
    DBDRV_UPD_Mid_WFMap = FUNCTION_RETURN_SUCCESS
...
PROC_ERR:
    ...
    Debug.Print "====== Error SQL ======"
    Debug.Print sql
    DBDRV_UPD_Mid_WFMap = FUNCTION_RETURN_FAILURE
    gErr.HandleError
    Resume proc_exit

End Function

'概要    :新サンプル管理(SXL)(中間抜試用)
'ﾊﾟﾗﾒｰﾀ  :変数名       ,IO   ,型                                    ,説明
'        :戻ﾘ値        ,O    ,FUNCTION_RETURN                       ,読み込み成否
'説明    :WFマップテーブル(TBCMY011)を更新する
'履歴    :
Public Function DBDRV_SAVE_XSDCW_1(pWafSmp() As typ_XSDCW, sErrMsg As String) As FUNCTION_RETURN
    Dim sql         As String
    Dim dynOra      As OraDynaset
    Dim sDbName     As String
    Dim i           As Integer
    ...
    '' エラーハンドラの設定
    On Error GoTo PROC_ERR
    gErr.Push "s_cmbc036_SQL.bas -- Function DBDRV_SAVE_XSDCW_1"
    sErrMsg = ""

    DBDRV_SAVE_XSDCW_1 = FUNCTION_RETURN_FAILURE

    sDbName = "XSDCW_1"

    '新サンプル管理(中間抜試用)にデータがあるか
    For i = 1 To UBound(pWafSmp)
        sql = "SELECT count(*) "
        sql = sql & "FROM  XSDCW_1 "
        sql = sql & "WHERE SXLIDCW ='" & pWafSmp(i).SXLIDCW & "'"
        sql = sql & "  and TBKBNCW ='" & pWafSmp(i).TBKBNCW & "'"
        sql = sql & "  and REPSMPLIDCW ='" & pWafSmp(i).REPSMPLIDCW & "'"

        Set dynOra = OraDB.DBCreateDynaset(sql, 0&)
        If dynOra.Fields(0) <> 0 Then
            'データがあればUpdate
            If DBDRV_Mid_WfSmp_UPD(pWafSmp(), i) = FUNCTION_RETURN_FAILURE Then
                sErrMsg = GetMsgStr("ENG11", vbNullString, sDbName)
                GoTo proc_exit
            End If
        Else
            'なければInsert
            If DBDRV_Mid_WfSmp_INS(pWafSmp(), i) = FUNCTION_RETURN_FAILURE Then
                sErrMsg = GetMsgStr("ENG11", vbNullString, sDbName)
                GoTo proc_exit
            End If
        End If
    ...
    DBDRV_SAVE_XSDCW_1 = FUNCTION_RETURN_SUCCESS
...
'概要      :WFサンプル管理(中間抜試)の挿入
'ﾊﾟﾗﾒｰﾀ　　:変数名      ,IO ,型                 ,説明
'      　　:WFSMP 　　　,I  ,typ_XSDCW   　     ,新サンプル管理（SXL）
'      　　:戻り値      ,O  ,FUNCTION_RETURN　  ,書き込みの成否
'説明      :DBDRV_WfSmp_UpdInsに移行する予定
'履歴      :
Public Function DBDRV_Mid_WfSmp_INS(WFSMP() As typ_XSDCW, i As Integer) As FUNCTION_RETURN
    ...
    '' エラーハンドラの設定
    On Error GoTo PROC_ERR
    gErr.Push "s_cmbc036_SQL.bas -- Function DBDRV_Mid_WfSmp_INS"

    DBDRV_Mid_WfSmp_INS = FUNCTION_RETURN_SUCCESS

    sDbName = "XSDCW_1"
        With WFSMP(i)

                sql = "insert into XSDCW_1 ("
                sql = sql & "SXLIDCW, "         ' SXLID
                sql = sql & "SMPKBNCW, "        ' サンプル区分
                sql = sql & "TBKBNCW, "         ' T/B区分
                sql = sql & "REPSMPLIDCW, "     ' サンプルID
...
'''''           sql = sql & .SMPLPATCW & "', '"         ' サンプルパターン
        ...
        End With
        If OraDB.ExecuteSQL(sql) <= 0 Then
            DBDRV_Mid_WfSmp_INS = FUNCTION_RETURN_FAILURE
            GoTo proc_exit
        End If
'    Next i
...
'概要      :WFサンプル管理(中間抜試)の更新
'ﾊﾟﾗﾒｰﾀ　　:変数名      ,IO ,型                 ,説明
'      　　:WFSMP 　　　,I  ,typ_XSDCW   　     ,新サンプル管理（SXL）
'      　　:戻り値      ,O  ,FUNCTION_RETURN　  ,書き込みの成否
'説明      :新サンプル管理のデータを更新する
'履歴      :
Public Function DBDRV_Mid_WfSmp_UPD(WFSMP() As typ_XSDCW, i As Integer) As FUNCTION_RETURN
    ...
    '' エラーハンドラの設定
    On Error GoTo PROC_ERR
    gErr.Push "s_cmbc036_SQL.bas -- Function DBDRV_Mid_WfSmp_UPD"

    DBDRV_Mid_WfSmp_UPD = FUNCTION_RETURN_SUCCESS
...
'               sql = sql & "SMPKBNCW='" & .SMPKBNCW & "'"              ' サンプル区分"
        ...
        End With
        If OraDB.ExecuteSQL(sql) <= 0 Then
            DBDRV_Mid_WfSmp_UPD = FUNCTION_RETURN_FAILURE
            GoTo proc_exit
        End If
'    Next i
