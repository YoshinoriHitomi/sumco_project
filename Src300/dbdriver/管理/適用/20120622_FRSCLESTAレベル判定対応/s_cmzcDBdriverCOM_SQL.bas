...
''Add Start 2011/07/14 LT10Ω判定追加対応 T.Koi(SETsw)
                    sql = sql & "CRYREST10CS='" & .CRYREST10CS & "', "      ' 実績FLG（T)
...
'                    'Add End   2010/12/13 SMPK Miyata
                    ...
                    'Add Start 2012/06/13 SMPK H.Ohkubo CLESTA評価レベル判定対応
                    If Trim(.HINBCS) <> "Z" Then
                        If left(.JDGEIDCS, 1) <> vbNullChar And Trim(.JDGEIDCS) <> "" Then
                            sql = sql & ",JDGEIDCS='" & .JDGEIDCS & "' "        ' 判定ID
                        Else
                            sql = sql & ",JDGEIDCS=NULL "                       ' 判定ID
                        End If
                    End If
                    'Add End 2012/06/13 SMPK H.Ohkubo CLESTA評価レベル判定対応
