...
'C－OSF3判定機能追加 2007/04/23 M.Kaga END   ---
            ...
            Next
'Cng Start 2012/05/30 SMPK H.Ohkubo CLESTA評価レベル判定対応
           sqlBase = sqlBase & "T.SPECRRNO, T.SXLMCNO, T.WFMCNO, T.STAFFID, T.REGDATE, T.UPDDATE, T.SENDFLAG, T.SENDDATE, U.COSF3FLAG, T.HSXCOSF3NS, T.HSXCOSF3PK "
...
'Cng End 2012/05/30 SMPK H.Ohkubo CLESTA評価レベル判定対応
...
'' 2008/10/01 L/DL,OSF判定ﾛｼﾞｯｸ追加 ADD By Systech End
            ...
            If fldNameExist("HSXCOSF3NS") Then
                If IsNull(rs("HSXCOSF3NS")) = False Then .HSXCOSF3NS = rs("HSXCOSF3NS")     '品ＳＸＣＯＳＦ３保証方法＿処
            End If
            'Add End   2011/02/17 Y.Hitomi
            
            'Add Start 2012/05/30 SMPK H.Ohkubo CLESTA評価レベル判定対応
            '品ＳＸＣＯＳＦ３パターン区分
            If fldNameExist("HSXCOSF3PK") Then
                If IsNull(rs("HSXCOSF3PK")) = False Then
                    If rs("HSXCOSF3PK") = " " Then
                        .HSXCOSF3PK = "4"   '不問
                    ElseIf rs("HSXCOSF3PK") Then
                        .HSXCOSF3PK = rs("HSXCOSF3PK")
                    End If
                Else
                    .HSXCOSF3PK = "4"       '不問
                End If
...
'Add Start 2012/05/17 判定ID追加    SMPK H.Ohkubo CLESTA評価レベル判定対応
    sqlBase = sqlBase & ",JDGEIDCS "
...
'Add Start 2012/05/17 SMPK H.Ohkubo CLESTA評価レベル判定対応
            If IsNull(rs("JDGEIDCS")) = False Then .JDGEIDCS = rs("JDGEIDCS")
'Add End 2012/05/17 SMPK H.Ohkubo CLESTA評価レベル判定対応
        End With
        rs.MoveNext
    Next
