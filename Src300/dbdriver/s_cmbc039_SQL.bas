...
End Type
Public typ_zero() As DBDRV_scmzc_fcmlc001b_SXL039   '0枚ロット一覧情報(待ち一覧定義を使用)　ADD 2012/09/07 SETsw Marushita
...
'>>>>>> 2012/09/07 10以下ロット流動対応 SETsw Marushita
'************************************************************************************
'*    関数名        : DBDRV_fcmbc039_6_Disp
'*
'*    処理概要      : WF総合判定 ０枚ロット一覧 表示用ＤＢドライバ
'*
'*    パラメータ    : 変数名       ,IO  ,型                            ,説明
'*              　　:sMuke　　     ,I   ,String                        ,向先
'*              　　:sDateF        ,I   ,String                        ,対象日付From
'*              　　:sDateT        ,I   ,String                        ,対象日付To
'*              　　:SXL           ,O   ,DBDRV_scmzc_fcmlc001b_SXL039  ,SXL管理用
'*              　　:sErrMsg 　　　,O   ,String                        ,エラーメッセージ
'*
'*    戻り値        : 正常終了時はFUNCTION_RETURN_SUCCESS(0),
...
'*
'************************************************************************************
Public Function DBDRV_fcmbc039_6_Disp(sMuke As String, sDateF As String, sDateT As String, _
                                            sxl() As DBDRV_scmzc_fcmlc001b_SXL039, _
                                            sErrMsg As String _
                                            ) As FUNCTION_RETURN
    Dim udtWKSXL()      As DBDRV_scmzc_fcmlc001b_SXL039
    ...
    Dim intSxlCount     As Integer
    Dim rs2             As OraDynaset
    Dim lngCmpCnt       As Long             'サンプル数
    
Debug.Print "1 " & Now & " 0枚ロット、XSDCBを取得 SQL実行"
    DBDRV_fcmbc039_6_Disp = FUNCTION_RETURN_SUCCESS
...
Debug.Print "2 " & Now & " SXLに対する新ｻﾝﾌﾟﾙ情報をセット"
    sSql = sSql & "select "
    sSql = sSql & "CRYNUM, "                        '結晶番号
    sSql = sSql & "INGOTPOS, "                      '結晶位置
    sSql = sSql & "LENGTH, "                        '長さ
    sSql = sSql & "SXLID, "                         'SXL-ID
    sSql = sSql & "KRPROCCD, "                      '工程
    sSql = sSql & "NOWPROC, "                       '現在工程
    sSql = sSql & "LPKRPROCCD, "                    '工程
    sSql = sSql & "LASTPASS, "                      '最終通過工程
    sSql = sSql & "DELCLS, "                        '生死フラグ
    sSql = sSql & "LSTATCLS, "                      '最終状態区分
    sSql = sSql & "HOLDCLS, "                       'ホールド区分(SXL確定)
    sSql = sSql & "HINBAN, "                        '品番
    sSql = sSql & "REVNUM, "                        '製品番号改訂番号
    sSql = sSql & "FACTORY, "                       '工場
    sSql = sSql & "OPECOND, "                       '操業条件
    ...
    sSql = sSql & "REGDATE, "                       '登録日付
    sSql = sSql & "UPDDATE, "                       '更新日付
    sSql = sSql & "HOLDBCB, "                       'ﾎｰﾙﾄﾞ区分
    sSql = sSql & "WFHOLDFLGCB, "                   'WFﾎｰﾙﾄﾞ区分
    sSql = sSql & "KBLKFLGCB, "                     '関連ﾌﾞﾛｯｸﾌﾗｸﾞ
    sSql = sSql & "PLANTCAT, "                      '向先
    sSql = sSql & "XTALCW, "                        '結晶番号
    sSql = sSql & "INPOSCW, "                       '結晶位置
    sSql = sSql & "nvl(TBKBNCW,'T') as TBKBNCW, "   'TB区分
    sSql = sSql & "SMPKBNCW, "                      'サンプル区分
    sSql = sSql & "REPSMPLIDCW, "                   '代表サンプルID
    sSql = sSql & "HINBCW, "                        '品番
    sSql = sSql & "REVNUMCW, "                      '製品番号改訂番号
    sSql = sSql & "FACTORYCW, "                     '工場
    sSql = sSql & "OPECW, "                         '操業条件
    ...
    sSql = sSql & "xtalcb as CRYNUM, "              '結晶番号
    sSql = sSql & "inposcb as INGOTPOS, "           '結晶内開始位置
    sSql = sSql & "LENCB as LENGTH, "               '理論長さ→長さ
    sSql = sSql & "sxlidcb as SXLID, "              'SXLID
    sSql = sSql & "' ' as KRPROCCD, "               '工程
    sSql = sSql & "gnwkntcb as NOWPROC, "           '現在工程
    sSql = sSql & "' ' as LPKRPROCCD, "             '工程
    sSql = sSql & "newkntcb as LASTPASS, "          '最終通過工程
    sSql = sSql & "livkcb as DELCLS, "              '生死フラグ
    sSql = sSql & "lstccb as LSTATCLS, "            '最終状態区分
    sSql = sSql & "sholdclscb HOLDCLS, "            'ホールド区分(SXL確定)
    sSql = sSql & "hinbcb as HINBAN, "              '品番
    sSql = sSql & "revnumcb as REVNUM, "            '成否院番号改訂番号
    sSql = sSql & "factorycb as FACTORY, "          '工場
    sSql = sSql & "opecb as OPECOND, "              '操業条件
    sSql = sSql & "MAICB, "                         '枚数
    sSql = sSql & "tdaycb as REGDATE, "             '登録日付
    sSql = sSql & "kdaycb as UPDDATE, "             '更新日付
    sSql = sSql & "HOLDBCB, "                       'ホールド区分
    sSql = sSql & "WFHOLDFLGCB, "                   'WFホールド区分
    sSql = sSql & "KBLKFLGCB, "                     '関連ﾌﾞﾛｯｸﾌﾗｸﾞ
    sSql = sSql & "PLANTCATCB  as PLANTCAT "        '向先
    sSql = sSql & "from XSDCB "
    sSql = sSql & "where MAICB = 0 "                '枚数=0
    If sDateF <> "" Then 'WF払出日付From
        sSql = sSql & "   AND TDAYCB > TO_DATE('" & sDateF & "','YYYY/MM/DD')"
    End If
    If sDateT <> "" Then 'WF払出日付To
        sSql = sSql & "   AND TDAYCB < TO_DATE('" & sDateT & "','YYYY/MM/DD HH24:MI:SS')"
    End If
    If sMuke <> "ALL" Then '向先
        sSql = sSql & "   AND PLANTCATCB      = '" & sMuke & "'"
    End If
    sSql = sSql & "and ZEROFLGCB = '1' "            '0枚ロットフラグ='1'
    sSql = sSql & "), "
...
proc_err:
    ...
    Debug.Print "====== Error SQL ======"
    Debug.Print sSql
    DBDRV_fcmbc039_6_Disp = FUNCTION_RETURN_FAILURE
    gErr.HandleError
    Resume proc_exit
End Function
