...
Public Type typ_cmkc001f_Disp
    ...
    '払出ロック機能対応 2012/07/13 SETsw Marushita
    PLANL_B             As Long                  ' 計画量(バスケット)
    PLANL_M             As Long                  ' 計画量(当月)
    HJITUL              As Long                  ' 払出実績量
    HLEN                As Long                  ' 払出長さ
    HLOCK               As Long                  ' 払出ロック
    ALARM               As Long                  ' アラーム
End Type
...
Public Type typ_cmkc001f_ExecCryIn
    ...
    ''--------- 2012/03/12 ADD ISHII Start
    KRPROCCD            As String * 5           ' 現在管理工程
    NOWPROC             As String * 5           ' 現在工程
    ''--------- 2012/03/12 ADD ISHII End
End Type
...
''--------- 2012/03/12 ADD ISHII Start
'' 結晶情報の更新
Public Function fcmkc001f_Exec_037_2(CryIn As typ_cmkc001f_ExecCryIn, typTBCME037 As typ_TBCME037) As FUNCTION_RETURN
    ...
    With typTBCME037
        ' 管理工程コード
        If .KRPROCCD <> "" And left(.KRPROCCD, 1) <> vbNullChar Then
            sql = sql & " , KRPROCCD  =" & "'" & .KRPROCCD & "'"
        End If
        
        ' 工程コード
        If .PROCCD <> "" And left(.PROCCD, 1) <> vbNullChar Then
            sql = sql & " , PROCCD    =" & "'" & .PROCCD & "'"
        End If
        
        ' 最終通過管理工程
        If .LPKRPROCCD <> "" And left(.LPKRPROCCD, 1) <> vbNullChar Then
            sql = sql & " , LPKRPROCCD    =" & "'" & .LPKRPROCCD & "'"
        End If
        
        ' 最終通過工程
        If .LASTPASS <> "" And left(.LASTPASS, 1) <> vbNullChar Then
            sql = sql & " , LASTPASS  =" & "'" & .LASTPASS & "'"
        End If
    End With
    
    ' Where
    sql = sql & " where CRYNUM=" & "'" & CryIn.CRYNUM & "'"
    '' WriteDBLog sql, sDbName
    
Debug.Print "fcmkc001f_Exec_037_2 " & sql

    If OraDB.ExecuteSQL(sql) <= 0 Then
        sErrMsg = GetMsgStr("ENG11", vbNullString, sDbName)
        fcmkc001f_Exec_037_2 = FUNCTION_RETURN_FAILURE
    Else
        fcmkc001f_Exec_037_2 = FUNCTION_RETURN_SUCCESS
    End If
End Function
''--------- 2012/03/12 ADD ISHII End
...
''--------- 2012/03/12 ADD ISHII Start
'工程戻し時メイン
Public Function DBDRV_fcmkc001f_ExecModoshi( _
  CryIn As typ_cmkc001f_ExecCryIn, _
  CryFTest As typ_cmkc001f_ExecFts, _
  BlockMan As typ_cmkc001f_Block _
  ) As FUNCTION_RETURN
    
    'エラーハンドラの設定
    On Error GoTo proc_err
    gErr.Push "s_cmzcF_cmkc001f_SQL.bas -- Function DBDRV_fcmkc001f_ExecModoshi"

    DBDRV_fcmkc001f_ExecModoshi = FUNCTION_RETURN_SUCCESS

    Dim typTBCME037 As typ_TBCME037     ' 結晶情報
    
    'ブロック管理テーブルの更新 TBCME040
    With BlockMan
        .KRPROCCD = CryFTest.KRPROCCD   ' 現在管理工程      ← 戻し先管理工程
        .NOWPROC = CryFTest.PROCCODE    ' 現在工程          ← 戻し先工程
        .LPKRPROCCD = CryIn.KRPROCCD    ' 最終通過管理工程  ← 現在管理工程
        .LASTPASS = CryIn.NOWPROC       ' 最終通過工程      ← 現在工程
        
        '' CC705 --> CC600 or CC705 --> CC700 or CC720 --> CC700
'        If (CryIn.NOWPROC = "CC705" Or CryIn.NOWPROC = "CC720") And CryFTest.PROCCODE = "CC700" Then
        If (CryIn.NOWPROC = "CC705" And CryFTest.PROCCODE = "CC600") Or _
           (CryIn.NOWPROC = "CC705" And CryFTest.PROCCODE = "CC700") Or _
           (CryIn.NOWPROC = "CC720" And CryFTest.PROCCODE = "CC700") Then       '2012/04/19 FAE)ooba
            .LSTATCLS = "T"      '最終状態区分
        End If
    End With
    
    If FUNCTION_RETURN_FAILURE = fcmkc001f_ExecBlock(CryIn, BlockMan) Then
        DBDRV_fcmkc001f_ExecModoshi = FUNCTION_RETURN_FAILURE
    End If

    '結晶情報の更新 TBCME037
    With typTBCME037
        .KRPROCCD = CryFTest.KRPROCCD   ' 管理工程コード    ← 戻し先管理工程
        .PROCCD = CryFTest.PROCCODE     ' 工程コード        ← 戻し先工程
        .LPKRPROCCD = CryIn.KRPROCCD    ' 最終通過管理工程  ← 現在管理工程
        .LASTPASS = CryIn.NOWPROC       ' 最終通過工程      ← 現在工程
    End With
    
    If FUNCTION_RETURN_FAILURE = fcmkc001f_Exec_037_2(CryIn, typTBCME037) Then
        DBDRV_fcmkc001f_ExecModoshi = FUNCTION_RETURN_FAILURE
    End If
...
''--------- 2012/03/12 ADD ISHII Start
'------------------------------------------------------------------------------------------------------------(作りり直しSTR)
'概要    :返品待ち一覧 初期表示用ＤＢドライバ
'ﾊﾟﾗﾒｰﾀ  :変数名       ,IO  ,型                                 ,説明
'        :records      ,IO  ,type_DBDRV_scmzc_fcmkc001b_Disp    ,初期表示用
'        :戻ﾘ値        ,O   ,FUNCTION_RETURN                    ,読み込み成否
'説明    :
'履歴    :
Public Function DBDRV_scmzc_fcmkc001b_Disp7(records() As type_DBDRV_scmzc_fcmkc001b_Disp5, tmpBlkData() As typ_BlkData, scmbKosei As Integer, stxtblk As String) As FUNCTION_RETURN

    '＜返品待ち一覧＞
    'エラーハンドラの設定
    On Error GoTo proc_err
    gErr.Push "s_cmbc032_SQL.bas -- Function DBDRV_scmzc_fcmkc001b_Disp7"

    DBDRV_scmzc_fcmkc001b_Disp7 = FUNCTION_RETURN_SUCCESS

    'ブロックID､更新日付、品番等取得
     If getBlockID2(records(), tmpBlkData(), "CC705", 3, scmbKosei, stxtblk) = FUNCTION_RETURN_FAILURE Then
        DBDRV_scmzc_fcmkc001b_Disp7 = FUNCTION_RETURN_FAILURE
        GoTo proc_exit
    End If
...
'    sql = sql & "   and XC2.CRYNUMC2 = X.CRYNUMCA "
    ...
    ElseIf formNum = 3 Then
        sql = sql & " and XC2.GNWKNTC2 = 'CC705'"
        sql = sql & " and XC2.NEWKNTC2 = '     '"
        sql = sql & " and XC2.LSTATBC2 = 'B'"
        sql = sql & " and XC2.GNWKKBC2 IN ( '2', '3' )"
    End If
    ''--------- 2012/03/12 ADD ISHII End
    ...
    If formNum = 2 Then
        ...
        ''--------- 2012/03/12 ADD ISHII Start
        ''3:出荷構成解除
        ElseIf scmbKosei = 2 Then
            sql = sql & " and XC2.GNWKKBC2 = '2' and XC2.NEWKNTC2 = 'CC700'"
        ''--------- 2012/03/12 ADD ISHII End
        Else
...
'
        ...
        ''--------- 2012/03/12 UPD ISHII Start
        'If rs("STOP") <> "2" And rs("WKKTY4") = IIf(formNum = 1, "CC720", IIf(formNum = 2, "CC705", "")) Then
        If rs("STOP") <> "2" And rs("WKKTY4") = IIf(formNum = 1, "CC720", IIf(formNum = 2 Or formNum = 3, "CC705", "")) Then
        ''--------- 2012/03/12 UPD ISHII End
           If Trim(records(j).AGRSTATUS) = "" Or (rs("AGRSTATUS") < records(j).AGRSTATUS) Then
                records(j).STOP = rs("STOP")                   '停止区分
                records(j).AGRSTATUS = rs("AGRSTATUS")         '承認確認区分
           End If
...
'    sql = sql & " order by B.BLOCKID, X.INPOSCA"
    ...
    ''--------- 2012/03/12 UPD ISHII Start
    ''ElseIf formNum = 2 Then
    ElseIf formNum = 2 Or formNum = 3 Then
    ''--------- 2012/03/12 UPD ISHII End
...
Public Function DBDRV_GetredXSDC3(records As typ_XSDC3_Update, t_CryNum As String, t_INPOS As Integer, Optional t_WKKTC3 As String = "") As FUNCTION_RETURN
''--------- 2012/03/12 UPD ISHII End
...
'概要      :返品ロット数取得
'ﾊﾟﾗﾒｰﾀ    :変数名        ,IO  ,型        ,説明
'          :plHenpinCnt   ,I   ,String    ,返品ロット数
'          :戻り値        ,O   ,Integer   ,処理成功：FUNCTION_RETURN_SUCCESS　処理失敗：FUNCTION_RETURN_FAILURE
'説明      :分割結晶ブロック(XSDC2)から返品ロットの数を取得する
'履歴      :2012/03/12 ISHII
Public Function DBDRV_GetHenpinLotCnt(ByRef plHenpinCnt As Long) As FUNCTION_RETURN
    Dim sql As String       'SQL全体
    Dim rs As OraDynaset    'RecordSet
    Dim recCnt As Long      'レコード数
    ...
    'エラーハンドラの設定
    On Error GoTo proc_err
    gErr.Push "s_cmbc032_SQL.bas -- Function DBDRV_GetHenpinLotQty"

     ''SQLを組み立てる
     sql = "SELECT  COUNT(ROWID) HENPIN_LOT_CNT"
     sql = sql & " FROM XSDC2 XC2"
     sql = sql & " WHERE XC2.GNWKNTC2 = 'CC705'"
     sql = sql & " AND   XC2.NEWKNTC2 = '     '"
     sql = sql & " AND   XC2.LSTATBC2 = 'B'"
     sql = sql & " AND   XC2.RSTATBC2 = 'T'"
     sql = sql & " AND   XC2.LIVKC2   = '0'"
     ...
     Debug.Print "返品ロット数取得SQL=" & sql
     
    ''データを抽出する
     Set rs = OraDB.DBCreateDynaset(sql, ORADYN_DEFAULT)
     If rs Is Nothing Then
         DBDRV_GetHenpinLotCnt = FUNCTION_RETURN_FAILURE
         Exit Function
     End If

    plHenpinCnt = rs("HENPIN_LOT_CNT")
    rs.Close

    DBDRV_GetHenpinLotCnt = FUNCTION_RETURN_SUCCESS
...
'>>>>> Add 2012/07/13 払出ロック機能対応 SETsw Marushita
'バスケット番号取得(XODM1から日付でバスケット番号を取得)
Public Function GetBasketNo(sDate As String) As Integer

    Dim sql As String
    Dim rs As OraDynaset

    On Error GoTo proc_err
    gErr.Push "s_cmbc032_SQL.bas -- Function GetBasketNo"
    
    GetBasketNo = 0
    
    sql = "select BSKTM1 from XODM1 "
    sql = sql & "where MONTHM1 = " & Mid(sDate, 5, 2)
    sql = sql & "and DATEM1 = " & Mid(sDate, 7, 2)
    ...
    If IsNull(rs("BSKTM1")) = False Then GetBasketNo = rs("BSKTM1")       'バスケット番号
