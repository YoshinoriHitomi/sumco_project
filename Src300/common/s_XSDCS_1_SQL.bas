...
Public Type typ_XSDCS_1
    ...
    'Add Start 2011/08/09 SMPK Nakamura FRS改善対応
    JUDGECNTFRSCS1  As String       ' 判定回数(FRS)
    'Add End 2011/08/09 SMPK Nakamura FRS改善対応
    CRYINDOIFRSCS1  As String       ' 状態FLG(FRS)
    CRYRESOIFRSCS1  As String       ' 実績FLG(FRS)
    'Add Start 2011/08/09 SMPK Nakamura FRS改善対応
    JUDGE1FRSCS1    As String       ' CLESTA判定結果(FRS)
    JUDGE2FRSCS1    As String       ' CLESTA-Z判定結果(FRS)
'    JUDGE3FRSCS1    As String       ' 判定結果3(FRS)
'    JUDGE4FRSCS1    As String       ' 判定結果4(FRS)
'    JUDGE5FRSCS1    As String       ' 判定結果5(FRS)
    'Add End 2011/08/09 SMPK Nakamura FRS改善対応
...
Public Function GetXSDCS_1(sBlock As String, typXSDCS1() As typ_XSDCS_1) As Boolean
    ...
    If CheckUniqueRecordXSDCS_1(sBlock) = False Then
        ...
        With typXSDCS1(1)
            ...
            'Add Start 2011/08/09 SMPK Nakamura FRS改善対応
            .JUDGECNTFRSCS1 = "0"                   '' 判定回数(FRS)
            'Add End 2011/08/09 SMPK Nakamura FRS改善対応
            .CRYINDOIFRSCS1 = "0"                   '' 状態FLG(FRS)
            .CRYRESOIFRSCS1 = "0"                   '' 実績FLG(FRS)
            'Add Start 2011/08/09 SMPK Nakamura FRS改善対応
            .JUDGE1FRSCS1 = "0"                     '' CLESTA判定結果(FRS)
            .JUDGE2FRSCS1 = "0"                     '' CLESTA-Z判定結果(FRS)
'            .JUDGE3FRSCS1 = "0"                     '' 判定結果3(FRS)
'            .JUDGE4FRSCS1 = "0"                     '' 判定結果4(FRS)
'            .JUDGE5FRSCS1 = "0"                     '' 判定結果5(FRS)
            'Add End 2011/08/09 SMPK Nakamura FRS改善対応
    ...
    'Add Start 2011/08/09 SMPK Nakamura FRS改善対応
    sSql = sSql & ", JUDGECNTFRSCS1"      '' 判定回数(FRS)
    'Add End 2011/08/09 SMPK Nakamura FRS改善対応
    sSQL = sSQL & ", CRYINDOIFRSCS1"      '' 状態FLG(FRS)
    sSQL = sSQL & ", CRYRESOIFRSCS1"      '' 実績FLG(FRS)
    'Add Start 2011/08/09 SMPK Nakamura FRS改善対応
    sSql = sSql & ", JUDGE1FRSCS1"        '' CLESTA判定結果(FRS)
    sSql = sSql & ", JUDGE2FRSCS1"        '' CLESTA-Z判定結果(FRS)
'    sSql = sSql & ", JUDGE3FRSCS1"        '' 判定結果3(FRS)
'    sSql = sSql & ", JUDGE4FRSCS1"        '' 判定結果4(FRS)
'    sSql = sSql & ", JUDGE5FRSCS1"        '' 判定結果5(FRS)
    'Add End 2011/08/09 SMPK Nakamura FRS改善対応
    ...
    If objDS.EOF = False Then
        Do Until objDS.EOF 'データがなくなるまで取得
            ...
            With typXSDCS1(lRecCnt)
                ...
                'Add Start 2011/08/09 SMPK Nakamura FRS改善対応
                If IsNull(objDS.Fields("JUDGECNTFRSCS1")) = False Then .JUDGECNTFRSCS1 = objDS.Fields("JUDGECNTFRSCS1")     '' 判定回数(FRS)
                'Add End 2011/08/09 SMPK Nakamura FRS改善対応
                ...
                'Add Start 2011/08/09 SMPK Nakamura FRS改善対応
                If IsNull(objDS.Fields("JUDGE1FRSCS1")) = False Then .JUDGE1FRSCS1 = objDS.Fields("JUDGE1FRSCS1")           '' CLESTA判定結果(FRS)
                If IsNull(objDS.Fields("JUDGE2FRSCS1")) = False Then .JUDGE2FRSCS1 = objDS.Fields("JUDGE2FRSCS1")           '' CLESTA-Z判定結果(FRS)
'                If IsNull(objDS.Fields("JUDGE3FRSCS1")) = False Then .JUDGE3FRSCS1 = objDS.Fields("JUDGE3FRSCS1")           '' 判定結果3(FRS)
'                If IsNull(objDS.Fields("JUDGE4FRSCS1")) = False Then .JUDGE4FRSCS1 = objDS.Fields("JUDGE4FRSCS1")           '' 判定結果4(FRS)
'                If IsNull(objDS.Fields("JUDGE5FRSCS1")) = False Then .JUDGE5FRSCS1 = objDS.Fields("JUDGE5FRSCS1")           '' 判定結果5(FRS)
                'Add End 2011/08/09 SMPK Nakamura FRS改善対応
...
'    sSql = sSql & ", JUDGE5FRSCS1"        '' 判定結果5(FRS)
    ...
    With pXSDCS_1
        ...
        'Add Start 2011/08/09 SMPK Nakamura FRS改善対応
        sSql = sSql & "," & Cnv2Number(.JUDGECNTFRSCS1) & ""                    '' 判定回数(FRS)
        'Add End 2011/08/09 SMPK Nakamura FRS改善対応
        sSQL = sSQL & "," & Cnv2String2(.CRYINDOIFRSCS1) & ""                   '' 状態FLG(FRS)
        sSQL = sSQL & "," & Cnv2String2(.CRYRESOIFRSCS1) & ""                   '' 実績FLG(FRS)
        'Add Start 2011/08/09 SMPK Nakamura FRS改善対応
        sSql = sSql & "," & Cnv2Number(.JUDGE1FRSCS1) & ""                      '' CLESTA判定結果(FRS)
        sSql = sSql & "," & Cnv2Number(.JUDGE2FRSCS1) & ""                      '' CLESTA-Z判定結果(FRS)
'        sSql = sSql & "," & Cnv2Number(.JUDGE3FRSCS1) & ""                      '' 判定結果3(FRS)
'        sSql = sSql & "," & Cnv2Number(.JUDGE4FRSCS1) & ""                      '' 判定結果4(FRS)
'        sSql = sSql & "," & Cnv2Number(.JUDGE5FRSCS1) & ""                      '' 判定結果5(FRS)
        'Add End 2011/08/09 SMPK Nakamura FRS改善対応
...
Public Function UpdateXSDCS_1(sBlock As String, records As typ_XSDCS_1) As Boolean
    ...
    With records
        ...
        'Add Start 2011/08/09 SMPK Nakamura FRS改善対応
        If .JUDGECNTFRSCS1 <> "" And left(.JUDGECNTFRSCS1, 1) <> vbNullChar Then sql = sql & ", JUDGECNTFRSCS1 = " & Cnv2Number(.JUDGECNTFRSCS1)    ' 判定回数(FRS)
        'Add End 2011/08/09 SMPK Nakamura FRS改善対応
        ...
        'Add Start 2011/08/09 SMPK Nakamura FRS改善対応
        If .JUDGE1FRSCS1 <> "" And left(.JUDGE1FRSCS1, 1) <> vbNullChar Then sql = sql & ", JUDGE1FRSCS1 = " & Cnv2Number(.JUDGE1FRSCS1)            ' CLESTA判定結果(FRS)
        If .JUDGE2FRSCS1 <> "" And left(.JUDGE2FRSCS1, 1) <> vbNullChar Then sql = sql & ", JUDGE2FRSCS1 = " & Cnv2Number(.JUDGE2FRSCS1)            ' CLESTA-Z判定結果(FRS)
'        If .JUDGE3FRSCS1 <> "" And left(.JUDGE3FRSCS1, 1) <> vbNullChar Then sql = sql & ", JUDGE3FRSCS1 = " & Cnv2Number(.JUDGE3FRSCS1)            ' 判定結果3(FRS)
'        If .JUDGE4FRSCS1 <> "" And left(.JUDGE4FRSCS1, 1) <> vbNullChar Then sql = sql & ", JUDGE4FRSCS1 = " & Cnv2Number(.JUDGE4FRSCS1)            ' 判定結果4(FRS)
'        If .JUDGE5FRSCS1 <> "" And left(.JUDGE5FRSCS1, 1) <> vbNullChar Then sql = sql & ", JUDGE5FRSCS1 = " & Cnv2Number(.JUDGE5FRSCS1)            ' 判定結果5(FRS)
        'Add End 2011/08/09 SMPK Nakamura FRS改善対応
...
'          :戻り値        ,O    ,Boolean        ,実行の成否
'説明      :
'Chg Start 2011/08/09 SMPK Nakamura FRS改善対応
...
'Chg End 2011/08/09 SMPK Nakamura FRS改善対応
    ...
    'Add Start 2011/08/09 SMPK Nakamura FRS改善対応
    tXSDCS_1.JUDGECNTFRSCS1 = sJudgeCnt                     '' 判定回数(FRS)
    'Add End 2011/08/09 SMPK Nakamura FRS改善対応
    tXSDCS_1.CRYINDOIFRSCS1 = sFRSKbn                       '' 状態FLG(FRS)
    tXSDCS_1.CRYRESOIFRSCS1 = sResult                       '' 実績FLG(FRS)
    'Add Start 2011/08/09 SMPK Nakamura FRS改善対応
    tXSDCS_1.JUDGE1FRSCS1 = "0"                             '' CLESTA判定結果(FRS)
    tXSDCS_1.JUDGE2FRSCS1 = "0"                             '' CLESTA-Z判定結果(FRS)
'    tXSDCS_1.JUDGE3FRSCS1 = "0"                             '' 判定結果3(FRS)
'    tXSDCS_1.JUDGE4FRSCS1 = "0"                             '' 判定結果4(FRS)
'    tXSDCS_1.JUDGE5FRSCS1 = "0"                             '' 判定結果5(FRS)
    'Add End 2011/08/09 SMPK Nakamura FRS改善対応
