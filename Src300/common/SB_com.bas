...
Public Type typ_SB_com_xodb5_osf31_Cudeco
    ...
    CJALLMINDIC5        As Integer          ' CJ 共通Disk半径下限           2012/05/17 SMPK H.Ohkubo CLESTA評価レベル判定対応
...
Public tKakuXInitG() As typ_WarpKakuData    ' 縦(Y)角度ﾃﾞｰﾀ(TBCMY018)
Public tKakuYInitG() As typ_WarpKakuData    ' 横(X)角度ﾃﾞｰﾀ(TBCMY018)
Public tKakuXMeasG() As typ_WarpKakuData    ' 縦(Y)角度ﾃﾞｰﾀ(表示/判定用)
Public tKakuYMeasG() As typ_WarpKakuData    ' 横(X)角度ﾃﾞｰﾀ(表示/判定用)
...
Public Type typ_xodc5_osf30
    JDGEIDC5 As String * 4          ' 判定ID
    SYNFLAGC5 As String * 1         ' 承認ﾌﾗｸﾞ
    YMKFLAGC5 As String * 1         ' 削除ﾌﾗｸﾞ
End Type
...
'Add Start 2012/05/29 SMPK H.Ohkubo CLESTA評価レベル判定対応
'概要      :XSDCSよりC－OSF3判定IDの獲得を行う
'ﾊﾟﾗﾒｰﾀ    :変数名          ,IO ,型               ,説明
'          :strXtalNo       ,I  ,String           ,結晶番号
'          :srrTBKbn        ,I  ,String           ,TB区分
'          :strJudgeID      ,O  ,String           ,C－OSF3判定ID
'説明      :
'履歴      :2012/05/29 SMPK H.Ohkubo
Public Function GetJDGEID_XSDCS(strXtalNo As String, srrTBKbn As String, ByRef strJudgeID As String) As FUNCTION_RETURN
    Dim dbIsMine    As Boolean
    Dim rs          As OraDynaset
    Dim sql         As String

    '初期化
    GetJDGEID_XSDCS = FUNCTION_RETURN_FAILURE
    ...
    ''XSDCSよりC－OSF3判定IDの獲得を行う
    sql = ""
    sql = sql & "select JDGEIDCS from XSDCS"
    sql = sql & " where CRYNUMCS ='" & Trim$(strXtalNo) & "'"
    sql = sql & " and   TBKBNCS ='" & Trim$(srrTBKbn) & "'"
    ...
    Else
        'C－OSF3判定IDがNULLの場合
        If Trim(rs("JDGEIDCS")) = "" Or IsNull(rs("JDGEIDCS")) Then
            strJudgeID = vbNullString
        Else
            strJudgeID = rs("JDGEIDCS")
        End If
    End If
    rs.Close
    ...
    '正常終了
    GetJDGEID_XSDCS = FUNCTION_RETURN_SUCCESS
...
'概要      :判定ID取得
'ﾊﾟﾗﾒｰﾀ　　:変数名          ,IO ,型                 ,説明
'　　      :strPGID         ,I  ,String             ,PGID
'          :intHinFlg       ,I  ,Integer            ,品番フラグ(0:12桁/1:8桁)
'　　      :strHinban       ,I  ,String       　    ,品番
'　　      :strXtalNo       ,I  ,String             ,結晶番号
'　　      :strJudgeID      ,O  ,String             ,判定ID
'          :戻り値          ,O  ,FUNCTION_RETURN    ,読み込み成否
'説明      :
'履歴      :2012/05/29 SMPK H.Ohkubo
Public Function GetJDGEID(strPGID As String, intHinFlg As Integer, tHINBAN As tFullHinban, StrHinban As String, strXtalNo As String, ByRef strJudgeID As String) As FUNCTION_RETURN
    Dim strC3Flg        As String       ' C-OSF3フラグ
    Dim strQLCode       As String       ' 品質レベルコード
    Dim strFPLNOC       As String       ' 炉No
    Dim strSetPGID      As String       ' 設定PGID
    Dim sql             As String
    Dim rs              As OraDynaset
    Dim recCnt          As Integer
    Dim tHinbanDt       As tFullHinban
    Dim rec030()        As typ_xodc5_osf30
    
    GetJDGEID = FUNCTION_RETURN_SUCCESS
    
    ''エラーハンドラの設定
    On Error GoTo proc_err
    gErr.Push "SB_com.bas -- Function GetJDGEID"
    
    strJudgeID = ""
    
    If intHinFlg = 0 Then
        tHinbanDt.hinban = tHINBAN.hinban
        tHinbanDt.mnorevno = tHINBAN.mnorevno
        tHinbanDt.factory = tHINBAN.factory
        tHinbanDt.opecond = tHINBAN.opecond
    Else
        '' 最新品番情報検索
        If GetLastHinban(StrHinban, tHinbanDt) = FUNCTION_RETURN_FAILURE Then
            ' 最新品番情報の検索エラーの場合
            Call MsgOut(0, "C-0SF3判定ID取得NG DB:TBCME018,E036", ERR_DISP)
            GetJDGEID = FUNCTION_RETURN_FAILURE
            Exit Function
        End If
    End If
    
    '' C-OSF3フラグ、品質レベルコード取得
    If GetQLCode(tHinbanDt.hinban, tHinbanDt.mnorevno, tHinbanDt.factory, tHinbanDt.opecond, strC3Flg, strQLCode) = FUNCTION_RETURN_SUCCESS Then

        '獲得したC-OSF3ﾌﾗｸﾞが"H"もしくは"S"の場合
        If Trim(strC3Flg) = "S" Or Trim(strC3Flg) = "H" Then

            '炉No
            strFPLNOC = Trim(left(strXtalNo, 3))
            
            'PGID
            If Len(Trim(strPGID)) > 0 Then
                strSetPGID = strPGID
            Else
                sql = "Select PGID "
                sql = sql & "from TBCME037 "
                sql = sql & "where CRYNUM = '" & left(Trim(strXtalNo), 9) & "000' "
            
                ''データを抽出する
                Set rs = OraDB.DBCreateDynaset(sql, ORADYN_DEFAULT)
                If rs.RecordCount = 0 Then
                    Call MsgOut(0, "C-0SF3判定ID取得PGIDエラー", ERR_DISP)
                    GetJDGEID = FUNCTION_RETURN_FAILURE
                    Exit Function
                Else
                    If IsNull(rs("PGID")) = False Then
                        strSetPGID = rs("PGID")
                    End If
                End If
            End If
            
            'XODC5_OSF30,XODC5_OSF32より判定ID取得
            If Get_XODC5_OSF30(strSetPGID, strFPLNOC, strQLCode, rec030()) <> FUNCTION_RETURN_SUCCESS Then
                Call MsgOut(0, "C-OSF3判定ID取得NG DB:XODC5_OSF30,OSF32", ERR_DISP)
                GetJDGEID = FUNCTION_RETURN_FAILURE
                Exit Function
            End If

            '取得した承認ﾌﾗｸﾞが0:未承認の場合
            If Trim(rec030(1).SYNFLAGC5) = "0" Or Trim(rec030(1).SYNFLAGC5) = "" Then
                Call MsgOut(0, "C-OSF3判定IDが未承認 【 " & rec030(1).JDGEIDC5 & " 】", ERR_DISP)
                GetJDGEID = FUNCTION_RETURN_FAILURE
                Exit Function
            End If

            '取得した判定IDがNULLだった場合
            If Trim(rec030(1).JDGEIDC5) = "" Then
                Call MsgOut(0, "C-0SF3判定ID取得NULL", ERR_DISP)
                GetJDGEID = FUNCTION_RETURN_FAILURE
                Exit Function
            Else
                '■判定ID設定
                strJudgeID = Trim(rec030(1).JDGEIDC5)
            End If
    
        End If
    
    Else
        ' C-OSF3フラグ、品質レベルコードの取得エラーの場合
        Call MsgOut(0, "C-0SF3判定ID取得NG DB:TBCME036", ERR_DISP)
        GetJDGEID = FUNCTION_RETURN_FAILURE
        Exit Function
    End If
...
'概要    :結晶内側管理(TBCME036)情報取得
'ﾊﾟﾗﾒｰﾀ  :変数名        ,IO  ,型                      ,説明
'        :strHinban     ,I   ,String                  ,ねらい品番
'        :strRPREVNUM   ,I   ,Integer                 ,ねらい品番製品番号改訂番号
'        :strRPFACT     ,I   ,String                  ,ねらい品番工場
'        :strRPOPCOND   ,I   ,String                  ,ねらい品番操業条件
'        :strCOSF3ID    ,O   ,String                  ,C-OSF3判定ﾌﾗｸﾞ
'        :strQLCode     ,O   ,String                  ,品質レベルコード
'        :戻り値        ,O   ,FUNCTION_RETURN         ,読み込み成否
'説明    :結晶内側管理(TBCME036)より、C-OSF3判定ﾌﾗｸﾞ、品質レベルコードを取得する
'履歴    :2012/05/29 SMPK H.Ohkubo
Private Function GetQLCode(StrHinban As String, intRPREVNUM As Integer, strRPFACT As String, strRPOPCOND As String, strCOSF3ID As String, strQLCode As String) As FUNCTION_RETURN
    Dim sql As String
    Dim rs As OraDynaset
    Dim recCnt As Integer
    ...
    'エラーハンドラの設定
    On Error GoTo proc_err
    gErr.Push "SB_com.bas -- Function GetQLCode"

    sql = "Select COSF3FLAG "
    sql = sql & ", QCLEVELCODE "
    sql = sql & "from TBCME036 "
    sql = sql & "where HINBAN = '" & Trim(StrHinban) & "' "
    sql = sql & "and MNOREVNO = '" & intRPREVNUM & "' "
    sql = sql & "and FACTORY = '" & Trim(strRPFACT) & "' "
    sql = sql & "and OPECOND = '" & Trim(strRPOPCOND) & "' "

    ''データを抽出する
    Set rs = OraDB.DBCreateDynaset(sql, ORADYN_DEFAULT)
    If rs.RecordCount = 0 Then
        GetQLCode = FUNCTION_RETURN_FAILURE
        GoTo proc_exit
    End If

    ''抽出結果を格納する
    If IsNull(rs("COSF3FLAG")) = False Then strCOSF3ID = rs("COSF3FLAG")       'C-OSF3ﾌﾗｸﾞ
    If IsNull(rs("QCLEVELCODE")) = False Then strQLCode = rs("QCLEVELCODE")    '品質レベルコード

    rs.Close

    GetQLCode = FUNCTION_RETURN_SUCCESS
...
'概要    :C-OSF3判定(引上条件)(XODC5_OSF30)情報取得
'ﾊﾟﾗﾒｰﾀ  :変数名        ,IO  ,型                      ,説明
'        :strPGID       ,I   ,String                  ,PGID
'        :strFPLNOC     ,I   ,String                  ,号機№(結晶番号頭3桁)
'        :strQLCode     ,I   ,String                  ,品質レベルコード
'        :rec030()      ,IO  ,typ_xodc5_osf30         ,XODC5_OSF30情報
'        :戻り値        ,O   ,FUNCTION_RETURN         ,読み込み成否
'履歴    :2012/05/29 SMPK H.Ohkubo
Private Function Get_XODC5_OSF30(ByVal strPGID As String _
                              , ByVal strFPLNOC As String _
                              , ByVal strQLCode As String _
                              , ByRef rec030() As typ_xodc5_osf30 _
                              ) As FUNCTION_RETURN
    ...
    'エラーハンドラの設定
    On Error GoTo proc_err
    gErr.Push "SB_com.bas -- Function Get_XODC5_OSF30"

    sql = "select C30.JDGEIDC5"
    sql = sql & ",C30.SYNFLAGC5"
    sql = sql & ",C32.FPLNOC5"
    sql = sql & "  from XODC5_OSF30 C30"
    sql = sql & "      ,XODC5_OSF32 C32"
    sql = sql & " where C32.PGIDC5 = '" & strPGID & "'"
    sql = sql & "   and C32.FPLNOC5 in ('999','" & strFPLNOC & "')"
    If strQLCode <> "" Then
        sql = sql & "   and C32.QCLVLC5 = '" & strQLCode & "'"
    Else
        sql = sql & "   and C32.QCLVLC5 = '99'"
    End If
    sql = sql & "   and C30.JDGEIDC5 = C32.JDGEIDC5"
    sql = sql & "   and C30.JUDGEFLGC5 = '2'"               '引上げ条件判定フラグ 2:PGID判定
    sql = sql & "   and C30.YMKFLAGC5 = '0'"                '有効／無効フラグ 0:有効
    sql = sql & " order by FPLNOC5"

    'データを抽出する
    Set rs = OraDB.DBCreateDynaset(sql, ORADYN_DEFAULT)
    If rs.RecordCount = 0 Then
        
        'Chg Start 2012/07/23 SMPK H.Ohkubo
'        Get_XODC5_OSF30 = FUNCTION_RETURN_FAILURE
'        GoTo proc_exit
        If strQLCode <> "" Then
            '' 品質レベルコード=99以外で対象データが無かった場合、品質レベルコード=99にて検索する。
            sql = ""
            sql = "select C30.JDGEIDC5"
            sql = sql & ",C30.SYNFLAGC5"
            sql = sql & ",C32.FPLNOC5"
        ...
        Else
            ...
            GoTo proc_exit
        End If
        'Chg End 2012/07/23 SMPK H.Ohkubo
    End If

    '構造体の要素を決定
    ReDim rec030(1)
    
    For lCnt = 1 To rs.RecordCount
        rec030(1).JDGEIDC5 = NulltoStr(rs("JDGEIDC5").Value)    '判定ID
        rec030(1).SYNFLAGC5 = NulltoStr(rs("SYNFLAGC5").Value)  '承認ﾌﾗｸﾞ
        If strFPLNOC = NulltoStr(rs("FPLNOC5").Value) Then
            '号機が一致するデータを優先(一致したら抜ける)
            Exit For
        End If
    Next lCnt
    ...
    Get_XODC5_OSF30 = FUNCTION_RETURN_SUCCESS
...
End Function
'Add End 2012/05/29 SMPK H.Ohkubo CLESTA評価レベル判定対応
...
'          :DMINC5        ,O  ,Long             ,Dのみ下限　'Add 2012/05/31 SMPK H.Ohkubo
'          :DRDMINC5      ,O  ,Long             ,共存D下限  'Add 2012/05/31 SMPK H.Ohkubo
'説明      :
'履歴      :2007/04/23 作成  加賀
'Cng Start 2012/05/31 SMPK H.Ohkubo
Public Function GetCOSF3PTN(C_JDGEIDC5 As String, C_POSC5 As Long, C_strChkR As String, C_strChkD As String, C_RMAXC5 As String, C_DMAXC5 As String, C_DRRMAXC5 As String, C_DRDMAXC5 As String, C_DMINC5 As String, C_DRDMINC5 As String) As FUNCTION_RETURN
...
'Cng End 2012/05/31 SMPK H.Ohkubo
Dim dbIsMine As Boolean
Dim rs As OraDynaset
Dim sql As String
    ...
    ElseIf C_strChkR = "D" Then
        ...
        sql = ""
        'Cng Start 2012/05/31 SMPK H.Ohkubo
        sql = "SELECT TO_CHAR(DMAXC5) as DMAXC5,TO_CHAR(DMINC5) as DMINC5 FROM(select MIN(POSC5) as W_POSC5 from(select POSC5 from XODC5_OSF31 where (trim(JDGEIDC5)='" & Trim$(C_JDGEIDC5) & "') and POSC5 >='" & Trim$(C_POSC5) & "')  ),XODC5_OSF31 WHERE POSC5 = W_POSC5 AND (trim(JDGEIDC5)='" & Trim$(C_JDGEIDC5) & "')"
    ...
    ElseIf C_strChkR = "R" And C_strChkD = "D" Then
        ...
        sql = ""
        'Cng Start 2012/05/31 SMPK H.Ohkubo
        sql = "SELECT TO_CHAR(DRRMAXC5) as DRRMAXC5,TO_CHAR(DRDMAXC5) as DRDMAXC5,TO_CHAR(DRDMINC5) as DRDMINC5 FROM(select MIN(POSC5) as W_POSC5 from(select POSC5 from XODC5_OSF31 where (trim(JDGEIDC5)='" & Trim$(C_JDGEIDC5) & "') and POSC5 >='" & Trim$(C_POSC5) & "')  ),XODC5_OSF31 WHERE POSC5 = W_POSC5 AND (trim(JDGEIDC5)='" & Trim$(C_JDGEIDC5) & "')"
    ...
    Else
        ...
        ElseIf C_strChkR = "D" Then
            ...
            Else
                C_DMAXC5 = rs("DMAXC5")
            End If
            'Add Start 2012/05/31 SMPK H.Ohkubo
            If Trim(rs("DMINC5")) = "" Or IsNull(rs("DMINC5")) Then
                C_DMINC5 = vbNullString
            Else
                C_DMINC5 = rs("DMINC5")
            End If
            'Add End 2012/05/31 SMPK H.Ohkubo
        ElseIf C_strChkR = "R" And C_strChkD = "D" Then
            ...
            End If
            'Add Start 2012/05/31 SMPK H.Ohkubo
            If Trim(rs("DRDMINC5")) = "" Or IsNull(rs("DRDMINC5")) Then
                C_DRDMINC5 = vbNullString
            Else
                C_DRDMINC5 = rs("DRDMINC5")
            End If
            'Add End 2012/05/31 SMPK H.Ohkubo
        End If
    End If
...
Dim sql As String
    ...
    '判定条件(XODC5_OSF31)より判定データの獲得を行う
    'ﾊﾟﾗﾒｰﾀのﾊﾟﾀｰﾝ区分によって処理分岐
'Chg Start 2012/05/17 SMPK H.Ohkubo CLESTA評価レベル判定対応
'    sql = " select CJDMAXPIC5, CJRMAXPIC5, CJDRMAXPIC5, CJALLMAXDIC5, CJALLMINRINC5"
    sql = " select CJDMAXPIC5, CJRMAXPIC5, CJDRMAXPIC5, CJALLMINDIC5, CJALLMAXDIC5, CJALLMINRINC5"
'Chg End 2012/05/17 SMPK H.Ohkubo CLESTA評価レベル判定対応
...
'Add Start 2012/05/17 SMPK H.Ohkubo CLESTA評価レベル判定対応
        ptyp_Ret.CJALLMINDIC5 = CInt(fncNullCheck(rs("CJALLMINDIC5")))      ' CJ 共通Disk半径下限
'Add End 2012/05/17 SMPK H.Ohkubo CLESTA評価レベル判定対応
