...
'           s_Order    : ソートの条件（"CTR02A9,CTR01A9"等） 省略可
'
...
'///////////////////////////////////////////////////
'概要      :スプレッドコントロールのコマンドボタンの状態を設定する
'ﾊﾟﾗﾒｰﾀ    :変数名        ,IO ,型                ,説明
...
Public Sub SpCtrlButtonColor(ctrlObj As vaSpread, ByVal col As Long, ByVal row As Long, ByVal col2 As Long, ByVal row2 As Long, ctrlState As enm_CtrlStateKind, Optional bClear As Boolean = False)
    ...
    Select Case ctrlState
        Case CTRL_DISABLE                             '' 編集不可の場合
            ctrlObj.TypeButtonColor = COLOR_DISABLE     '' 背景色を表示項目色にする
            ctrlObj.Lock = True                         '' ロックする
        Case CTRL_DISABLE_GRAY                        '' 編集不可(グレー色表示)の場合
            ctrlObj.TypeButtonColor = COLOR_GRAY      '' 背景色をグレー色にする
            ctrlObj.Lock = True                         '' ロックする
        Case CTRL_DISABLE_SKY                         '' 編集不可(推定値)の場合
            ctrlObj.TypeButtonColor = COLOR_SKY         '' 背景色を推定色にする
            ctrlObj.Lock = True                         '' ロックする
        Case CTRL_WARNING                             '' 警告指示の場合
            ctrlObj.TypeButtonColor = COLOR_WARNING     '' 背景色を赤色表示にする
            ctrlObj.Lock = False                        '' ロックしない
        Case CTRL_DISABLE_WARNING                     '' 警告指示編集不可の場合
            ctrlObj.TypeButtonColor = COLOR_WARNING     '' 背景色を赤色表示にする
            ctrlObj.Lock = True                         '' ロックする
        Case CTRL_SELECTED
            ctrlObj.TypeButtonColor = COLOR_SELECTED  '' 背景色を選択色にする
            ctrlObj.Lock = True                         '' ロックする
        Case CTRL_ENABLE_GRAY                         '' 編集可(グレー色表示)の場合
            ctrlObj.TypeButtonColor = COLOR_GRAY        '' 背景色をグレー色にする
            ctrlObj.Lock = False                        '' ロックしない
        Case CTRL_ENABLE_YELLOW
            ctrlObj.TypeButtonColor = COLOR_YELLOW      '' コマンドボタンをイエローにする
            ctrlObj.Lock = False                        '' ロックしない
        ...
        Case CTRL_ENABLE_RED
            ctrlObj.TypeButtonColor = COLOR_RED         '' コマンドボタンをレッドにする
            ctrlObj.Lock = False                        '' ロックしない
        ...
        Case Else                                     '' その他の状態指定の場合
            ctrlObj.TypeButtonColor = COLOR_ENABLE      '' 背景色を白色表示にする
            ctrlObj.Lock = False                        '' ロックしない
    End Select
...
'///////////////////////////////////////////////////
'概要      :コマンドラインから結晶番号を取得する
' 返り値  :　結晶番号
'
' 引き数  :　無し
'
' 機能説明: コマンドラインより結晶番号を取得する
'///////////////////////////////////////////////////
