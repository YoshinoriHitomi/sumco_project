...
'Add Start 2011/03/10 SMPK Miyata
    MSMPFLGWFR      As String * 1   ' 中間抜試フラグ/WF/比抵抗
    MSMPFLGWFO      As String * 1   ' 中間抜試フラグ/WF/酸素
    MSMPFLGWFOF     As String * 1   ' 中間抜試フラグ/WF/OSF
    MSMPFLGWFBM     As String * 1   ' 中間抜試フラグ/WF/BMD
    MSMPFLGWFGD     As String * 1   ' 中間抜試フラグ/WF/GD
    MSMPFLGWFDOI    As String * 1   ' 中間抜試フラグ/WF/Δoi（酸素析出）
    MSMPFLGWFAOI    As String * 1   ' 中間抜試フラグ/WF/AOI（残存酸素）
    MSMPFLGWFDS     As String * 1   ' 中間抜試フラグ/WF/DSOD
    MSMPFLGWFDZ     As String * 1   ' 中間抜試フラグ/WF/DZ
    MSMPFLGEPOF     As String * 1   ' 中間抜試フラグ/EP/OSF
    MSMPFLGEPBM     As String * 1   ' 中間抜試フラグ/EP/BMD
    MSMPFLG         As String * 1   ' 中間抜試フラグ
    MSMPTANIMAI     As Integer      ' 中間抜試単位(枚数)
    MSMPCONSTMAI    As Integer      ' 中間抜試許容値(枚数)
...
Debug.Print "2-1 " & Now & " 新サンプル管理(SXL)を取得 SQL実行2"
    ' 新サンプル管理(SXL)を取得 中間抜試験サンプル用
    sSQL = "select SXLIDCW, SMPKBNCW, TBKBNCW, REVNUMCW, XTALCW, INPOSCW, REPSMPLIDCW, HINBCW, FACTORYCW, OPECW, KTKBNCW, SMCRYNUMCW, "
    sSQL = sSQL & "WFSMPLIDRSCW, WFSMPLIDRS1CW, WFSMPLIDRS2CW, WFINDRSCW, WFRESRS1CW, WFRESRS2CW, WFSMPLIDOICW, WFINDOICW, WFRESOICW, "
    sSQL = sSQL & "WFSMPLIDB1CW, WFINDB1CW, WFRESB1CW, WFSMPLIDB2CW, WFINDB2CW, WFRESB2CW, WFSMPLIDB3CW, WFINDB3CW, WFRESB3CW, "
    sSQL = sSQL & "WFSMPLIDL1CW, WFINDL1CW, WFRESL1CW, WFSMPLIDL2CW, WFINDL2CW, WFRESL2CW, WFSMPLIDL3CW, WFINDL3CW, WFRESL3CW, "
    ...
    '　TOP、BOTのサンプルの後ろに中間抜試験のサンプル管理を追加する
    intRecCnt = rs.RecordCount
    ReDim Preserve udtSXL.WFSMP(2 + intRecCnt)
    For i = 1 To intRecCnt
        With udtSXL.WFSMP(2 + i)
            .SXLIDCW = rs("SXLIDCW")                                                            ' SXL-ID
            .SMPKBNCW = rs("SMPKBNCW")                                                          ' サンプル区分
            .TBKBNCW = rs("TBKBNCW")                                                            ' T/B区分
...
'*               　　intPos       ,I   ,Integer                            ,ｻﾝﾌﾟﾙ位置　'Add 2011/03/07 SMPK Miyata
...
'*
'*******************************************************************************
Public Function funWfcGetDataEtc(udtTypIn As type_DBDRV_scmzc_fcmlc001c_In, intPos As Integer, udtNew_Hinban As tFullHinban, intSmpGetFlg As Integer, _
...
'    Dim intPos      As Integer      ' ｻﾝﾌﾟﾙ位置        'Del Start 2011/03/07 SMPK Miyata
...
'Add Start 2011/03/10 SMPK Miyata
    sSQL = sSQL & ", NVL(MSMPFLGWFR,'0')   as MSMPFLGWFR"   ' 中間抜試フラグ/WF/比抵抗
    sSQL = sSQL & ", NVL(MSMPFLGWFO,'0')   as MSMPFLGWFO"   ' 中間抜試フラグ/WF/酸素
    sSQL = sSQL & ", NVL(MSMPFLGWFOF,'0')  as MSMPFLGWFOF"  ' 中間抜試フラグ/WF/OSF
    sSQL = sSQL & ", NVL(MSMPFLGWFBM,'0')  as MSMPFLGWFBM"  ' 中間抜試フラグ/WF/BMD
    sSQL = sSQL & ", NVL(MSMPFLGWFGD,'0')  as MSMPFLGWFGD"  ' 中間抜試フラグ/WF/GD
    sSQL = sSQL & ", NVL(MSMPFLGWFDOI,'0') as MSMPFLGWFDOI" ' 中間抜試フラグ/WF/Δoi（酸素析出）
    sSQL = sSQL & ", NVL(MSMPFLGWFAOI,'0') as MSMPFLGWFAOI" ' 中間抜試フラグ/WF/AOI（残存酸素）
    sSQL = sSQL & ", NVL(MSMPFLGWFDS,'0')  as MSMPFLGWFDS"  ' 中間抜試フラグ/WF/DSOD
    sSQL = sSQL & ", NVL(MSMPFLGWFDZ,'0')  as MSMPFLGWFDZ"  ' 中間抜試フラグ/WF/DZ
    sSQL = sSQL & ", NVL(MSMPFLGEPOF,'0')  as MSMPFLGEPOF"  ' 中間抜試フラグ/EP/OSF
    sSQL = sSQL & ", NVL(MSMPFLGEPBM,'0')  as MSMPFLGEPBM"  ' 中間抜試フラグ/EP/BMD
    sSQL = sSQL & ", NVL(MSMPFLG,'0')      as MSMPFLG"      ' 中間抜試フラグ
    sSQL = sSQL & ", NVL(MSMPTANIMAI,0)    as MSMPTANIMAI"  ' 中間抜試単位(枚数)
    sSQL = sSQL & ", NVL(MSMPCONSTMAI,0)   as MSMPCONSTMAI" ' 中間抜試許容値(枚数)
'Add End   2011/03/10 SMPK Miyata
...
'Add Start 2011/03/10 SMPK Miyata
        .MSMPFLGWFR = rs("MSMPFLGWFR")                  ' 中間抜試フラグ/WF/比抵抗
        .MSMPFLGWFO = rs("MSMPFLGWFO")                  ' 中間抜試フラグ/WF/酸素
        .MSMPFLGWFOF = rs("MSMPFLGWFOF")                ' 中間抜試フラグ/WF/OSF
        .MSMPFLGWFBM = rs("MSMPFLGWFBM")                ' 中間抜試フラグ/WF/BMD
        .MSMPFLGWFGD = rs("MSMPFLGWFGD")                ' 中間抜試フラグ/WF/GD
        .MSMPFLGWFDOI = rs("MSMPFLGWFDOI")              ' 中間抜試フラグ/WF/Δoi（酸素析出）
        .MSMPFLGWFAOI = rs("MSMPFLGWFAOI")              ' 中間抜試フラグ/WF/AOI（残存酸素）
        .MSMPFLGWFDS = rs("MSMPFLGWFDS")                ' 中間抜試フラグ/WF/DSOD
        .MSMPFLGWFDZ = rs("MSMPFLGWFDZ")                ' 中間抜試フラグ/WF/DZ
        .MSMPFLGEPOF = rs("MSMPFLGEPOF")                ' 中間抜試フラグ/EP/OSF
        .MSMPFLGEPBM = rs("MSMPFLGEPBM")                ' 中間抜試フラグ/EP/BMD
...
'Del End   2011/03/07 SMPK Miyata
        ...
        If udtTypIn.WFSMP.WFHSGDCW = "1" Then
            ReDim Preserve typ_J015_WFGDJudg(intPos)    'Add 2011/03/07 SMPK Miyata
            sDBName = "J006"
...
'Del End   2011/03/07 SMPK Miyata
    ...
    If udtTypIn.WFSMP.WFINDL4CW <> "0" Then
        ReDim Preserve typ_J022_WFSDJudg(intPos)    'Add 2011/03/07 SMPK Miyata
...
'Del End   2011/03/07 SMPK Miyata
    ...
    If udtTypIn.WFSMP.WFINDSPCW <> "0" Then
        ReDim Preserve typ_J016_WFSPVJudg(intPos)    'Add 2011/03/07 SMPK Miyata
...
'*********************************************************************************************
'*    関数名        : funGetSiyou_WFXtalInclination
'*
'*    処理概要      : 1.WF結晶面傾き仕様値の取得処理
'*
'*    パラメータ    : 変数名    ,IO ,型           ,説明
'*                   strKubun   ,I  ,String       ,取得区分-XY(合成角)、X(横)、Y(縦)
'*                   udtHIN     ,I  ,tFullHinban  ,品番
'*                   dblKakuMin ,I  ,Double       ,結晶面傾下限
'*                   dblWarpMax ,I  ,Double       ,結晶面傾上限
'*
...
'*
'*********************************************************************************************
Public Function funGetSiyou_WFXtalInclination(tKubun As String, udtHin As tFullHinban, dblKakuMin As Double, dblKakuMax As Double) As FUNCTION_RETURN
    ...
    'エラーハンドラの設定
    On Error GoTo proc_err
'    gErr.Push "SB_WfJudg_SQL.bas -- Function funGetSiyou_WFXtalInclination"

    sql = "select HINBAN, MNOREVNO, FACTORY, OPECOND, "
    If tKubun = "X" Then
        sql = sql & "HWFCSXMIN as HWFCSMIN, "
        sql = sql & "HWFCSXMAX as HWFCSMAX "
    ElseIf tKubun = "Y" Then
        sql = sql & "HWFCSYMIN as HWFCSMIN, "
        sql = sql & "HWFCSYMAX as HWFCSMAX "
    Else
        sql = sql & "HWFCSGMIN as HWFCSMIN, "
        sql = sql & "HWFCSGMAX as HWFCSMAX "
    End If
    sql = sql & "from TBCME027 "
    sql = sql & "Where HINBAN = '" & udtHin.hinban & "' and "
    sql = sql & "      MNOREVNO = " & udtHin.mnorevno & " and "
    sql = sql & "      FACTORY = '" & udtHin.FACTORY & "' and "
    sql = sql & "      OPECOND = '" & udtHin.OPECOND & "'"
    
    ''データを抽出する
    Set rs = OraDB.DBCreateDynaset(sql, ORADYN_NO_BLANKSTRIP)
    If rs.RecordCount <> 1 Then
        funGetSiyou_WFXtalInclination = FUNCTION_RETURN_FAILURE
        GoTo proc_exit
    End If
    ...
    funGetSiyou_WFXtalInclination = FUNCTION_RETURN_SUCCESS
...
PROC_ERR:
    ...
    Resume proc_exit
End Function
'Add Start 2011/07/25 Y.Hitomi
'*******************************************************************************
'*    関数名        : ChkSXL_XSDCW_1
'*
'*    処理概要      : 対象シングルIDがXSDCW_1に存在するかチェックする
'*
'*    パラメータ    : 変数名        ,IO ,型           ,説明
...
'*
'*******************************************************************************
Public Function ChkSXL_XSDCW_1(ByVal sSXLID As String) As FUNCTION_RETURN
    Dim rs          As OraDynaset
    Dim sSQL        As String
    ...
    '-------------------- 初期ｸﾘｱ ----------------------------------------
    ChkSXL_XSDCW_1 = FUNCTION_RETURN_FAILURE

    sSQL = "select SXLIDCW from XSDCW_1 where SXLIDCW = '" & sSXLID & "' and LIVKCW = '0'"
    Set rs = OraDB.CreateDynaset(sSQL, ORADYN_NO_BLANKSTRIP)
    ...
    Set rs = Nothing
    ChkSXL_XSDCW_1 = FUNCTION_RETURN_SUCCESS
...
'*    関数名        : ChkMidSpec
'*
'*    処理概要      : 対象品番が製品仕様の中間抜試品かチェックする
'*
'*    パラメータ    : 変数名        ,IO ,型           ,説明
'*                    sHinban       ,I  ,STRING       ,品番
'*                    sOpecond      ,I  ,STRING       ,製作条件NO
'*
'*    戻り値        : 製品仕様の中間抜試品は    True
'*                    それ以外は、              False
'*
'*******************************************************************************
Public Function ChkMidSpec(ByVal sHinban As String, ByVal sOpecond As String) As Boolean
    Dim rs          As OraDynaset
    Dim sSQL        As String
    ...
    ChkMidSpec = False
    '' TBCME021
    '' 品ＷＦ比抵抗検査頻度＿保     HWFRKHNH
    '' TBCME024
    '' 品ＷＦ無欠陥層検査頻度＿保   HWFMKKHH
    ''
    '' TBCME025
    '' 品ＷＦ酸素濃度検査頻度＿保   HWFONKHH
    '' 品ＷＦ酸素析出１検査頻度＿保 HWFOS1KH
    '' 品ＷＦ酸素析出２検査頻度＿保 HWFOS2KH
    '' 品ＷＦ酸素析出３検査頻度＿保 HWFOS3KH
    '' 品ＷＦ残存酸素検査頻度＿保   HWFZOKHH
    ''
    '' TBCME026
    '' 品ＷＦＤＳＯＤ検査頻度＿保   HWFDSOKH
    ''
    '' TBCME029
    '' 品ＷＦＯＳＦ１検査頻度＿保   HWFOF1KH
    '' 品ＷＦＯＳＦ２検査頻度＿保   HWFOF2KH
    '' 品ＷＦＯＳＦ３検査頻度＿保   HWFOF3KH
    '' 品ＷＦＯＳＦ４検査頻度＿保   HWFOF4KH
    '' 品ＷＦＢＭＤ１検査頻度＿保   HWFBM1KH
    '' 品ＷＦＢＭＤ２検査頻度＿保   HWFBM2KH
    '' 品ＷＦＢＭＤ３検査頻度＿保   HWFBM3KH
    ''
    '' TBCME050
    '' 品ＥＰＯＳＦ１検査頻度＿保   HEPOF1KH
    '' 品ＥＰＯＳＦ２検査頻度＿保   HEPOF2KH
    '' 品ＥＰＯＳＦ３検査頻度＿保   HEPOF3KH
    '' 品ＥＰＢＭＤ１検査頻度＿保   HEPBM1KH
    '' 品ＥＰＢＭＤ２検査頻度＿保   HEPBM2KH
    '' 品ＥＰＢＭＤ３検査頻度＿保   HEPBM3KH
    
    sSQL = sSQL & "select  "
    sSQL = sSQL & " e021.hinban, "
    sSQL = sSQL & " e021.OPECOND, "
    sSQL = sSQL & "      instr( "
    sSQL = sSQL & "         nvl(HWFRKHNH,' ') || nvl(HWFMKKHH,' ') || nvl(HWFONKHH,' ') || nvl(HWFOS1KH,' ') ||     "
    sSQL = sSQL & "         nvl(HWFOS2KH,' ') || nvl(HWFOS3KH,' ') || nvl(HWFZOKHH,' ') || nvl(HWFDSOKH,' ') ||     "
    sSQL = sSQL & "         nvl(HWFOF1KH,' ') || nvl(HWFOF2KH,' ') || nvl(HWFOF3KH,' ') || nvl(HWFOF4KH,' ') ||     "
    sSQL = sSQL & "         nvl(HWFBM1KH,' ') || nvl(HWFBM2KH,' ') || nvl(HWFBM3KH,' ') || nvl(HEPOF1KH,' ') ||     "
    sSQL = sSQL & "         nvl(HEPOF2KH,' ') || nvl(HEPOF3KH,' ') || nvl(HEPBM1KH,' ') || nvl(HEPBM2KH,' ') ||     "
    sSQL = sSQL & "         nvl(HEPBM3KH,' ')   "
    sSQL = sSQL & "        ,'M') as HOSHO1, "
    sSQL = sSQL & "      instr( "
    sSQL = sSQL & "         nvl(HWFRKHNH,' ') || nvl(HWFMKKHH,' ') || nvl(HWFONKHH,' ') || nvl(HWFOS1KH,' ') ||     "
    sSQL = sSQL & "         nvl(HWFOS2KH,' ') || nvl(HWFOS3KH,' ') || nvl(HWFZOKHH,' ') || nvl(HWFDSOKH,' ') ||     "
    sSQL = sSQL & "         nvl(HWFOF1KH,' ') || nvl(HWFOF2KH,' ') || nvl(HWFOF3KH,' ') || nvl(HWFOF4KH,' ') ||     "
    ...
    sSQL = sSQL & "         nvl(HEPOF2KH,' ') || nvl(HEPOF3KH,' ') || nvl(HEPBM1KH,' ') || nvl(HEPBM2KH,' ') ||     "
    sSQL = sSQL & "         nvl(HEPBM3KH,' ')   "
    sSql = sSql & "        ,'N') as HOSHO2 "
    sSQL = sSQL & "from "
    sSQL = sSQL & " tbcme021 e021, "
    sSQL = sSQL & " tbcme024 e024, "
    sSQL = sSQL & " tbcme025 e025, "
    sSQL = sSQL & " tbcme026 e026, "
    sSQL = sSQL & " tbcme029 e029, "
    sSQL = sSQL & " tbcme050 e050 "
    sSQL = sSQL & "where "
    sSQL = sSQL & " e021.hinban = e024.hinban and "
    sSQL = sSQL & " e021.hinban = e025.hinban and "
    sSQL = sSQL & " e021.hinban = e026.hinban and "
    sSQL = sSQL & " e021.hinban = e029.hinban and "
    sSQL = sSQL & " e021.hinban = e050.hinban and "
    sSQL = sSQL & " e021.OPECOND = e024.OPECOND and "
    sSQL = sSQL & " e021.OPECOND = e025.OPECOND and "
    sSQL = sSQL & " e021.OPECOND = e026.OPECOND and "
    sSQL = sSQL & " e021.OPECOND = e029.OPECOND and "
    sSQL = sSQL & " e021.OPECOND = e050.OPECOND and "
    sSQL = sSQL & " e021.hinban  = '" & sHinban & "'  and "
    sSQL = sSQL & " e021.OPECOND = '" & sOpecond & "' "
    ...
    If rs("HOSHO1") <> 0 Or rs("HOSHO2") <> 0 Then
        ChkMidSpec = True
    End If
...
proc_err:
    ...
    Debug.Print "====== Error SQL ======"
    Debug.Print sSQL
    ChkMidSpec = FUNCTION_RETURN_FAILURE
    Resume proc_exit
