...
'Chg Start 2012/05/29 SMPK H.Ohkubo CLESTA評価レベル判定対応
...
'
'      ''Add End   2011/01/17 SMPK A.Nagamine
        Call CuDecoDataSet_C(JudgSC_B, typ_b, tt, i, typTmList(), sHinban12, typ_b.typ_si(tt).PGID)
        
        Call CuDecoDataSet_CJ(JudgSC_B, typ_b, tt, i, typTmList(), sHinban12, typ_b.typ_si(tt).PGID)
        
        Call CuDecoDataSet_CJLT(JudgSC_B, typ_b, tt, i, typTmList(), sHinban12, typ_b.typ_si(tt).PGID)
        
        Call CuDecoDataSet_CJ2(JudgSC_B, typ_b, tt, i, typTmList(), sHinban12, typ_b.typ_si(tt).PGID)
'Chg End 2012/05/29 SMPK H.Ohkubo CLESTA評価レベル判定対応
...
'Add Start 2012/05/22 SMPK H.Ohkubo CLESTA評価レベル判定対応
    CJ.CJALLMINDIC5 = typ_cjz.CJALLMINDIC5      ''CJ 共通Disk半径下限値
...
'Add Start 2012/05/29 SMPK H.Ohkubo CLESTA評価レベル判定対応
    Dim tDmyHinDt       As tFullHinban
    Dim strDMINC5       As String
    Dim strDRDMINC5     As String
    Dim lngDMINC5       As Long
    Dim lngDRDMINC5     As Long
...
'               If GetCOSF3ID(strJDGEIDC, strXTALC1) <> FUNCTION_RETURN_SUCCESS Then
                'C－OSF3判定IDを獲得する
               If GetJDGEID(typ_b.typ_si(UpDo).PGID, 1, tDmyHinDt, Trim(left(sHinban12, 8)), strXTALC1, strJDGEIDC) <> FUNCTION_RETURN_SUCCESS Then
'Chg End 2012/05/29 SMPK H.Ohkubo CLESTA評価レベル判定対応
                    ErrFlg = False
               Else
                   ...
                   If YFlg = False Then
                       ...
                       If strOSFRD1 = "R" And strOSFRD2 = "-" Then
                          ...
                          'Cng Start 2012/05/31 SMPK H.Ohkubo
                           If GetCOSF3PTN(strJDGEIDC, lngSMPPOS, strOSFRD1, strOSFRD2, strRMAXC5, strDMAXC5, strDRRMAXC5, strDRDMAXC5, strDMINC5, strDRDMINC5) <> FUNCTION_RETURN_SUCCESS Then
...
''Cng Start 2012/05/31 SMPK H.Ohkubo
                       ...
                       ElseIf strOSFRD1 = "D" Then
                           ...
                           If Trim(strDMAXC5) = "" Or Trim(strDMINC5) = "" Then
                               ErrFlg = False
                           Else
                                lngDMAXC5 = Trim(strDMAXC5)
                                lngDMINC5 = Trim(strDMINC5)
                                '実績値の判定
                                If (lngOSFWID1 >= lngDMINC5) And (lngOSFWID1 <= lngDMAXC5) Then
                                    '判定OK
                                    bJudg = True
                                ElseIf (lngOSFWID1 < lngDMINC5) Or (lngOSFWID1 > lngDMAXC5) Then
                                    '判定NG
                                    bJudg = False
                                End If
                           End If
                       ...
                       ElseIf strOSFRD1 = "R" And strOSFRD2 = "D" Then
                           ...
                           'ﾚｺｰﾄﾞ無又はﾏｽﾀの実績値がNULL：VBエラー(後で考える)
                           'Add Start 2012/06/29  Y.Hitomi
                           If Trim(strDRRMAXC5) = "" Or Trim(strDRDMAXC5) = "" Or Trim(strDRDMINC5) = "" Then
                          'If Trim(strDRRMAXC5) = "" Or Trim(strDRDMAXC5) = "" Or Trim(strDMINC5) = "" Then
                          'Add Start 2012/06/29  Y.Hitomi
                               ErrFlg = False
                           Else
                               lngDRRMAXC5 = Trim(strDRRMAXC5)
                               lngDRDMAXC5 = Trim(strDRDMAXC5)
                               lngDRDMINC5 = Trim(strDRDMINC5)
                               '実績値の判定
                               If lngOSFWID1 <= lngDRRMAXC5 And lngOSFWID2 >= lngDRDMINC5 And lngOSFWID2 <= lngDRDMAXC5 Then
                                   '判定OK
                                   bJudg = True
                               ElseIf lngOSFWID1 > lngDRRMAXC5 Or lngOSFWID2 < lngDRDMINC5 Or lngOSFWID2 > lngDRDMAXC5 Then
                                   '判定NG
                                   bJudg = False
                               End If
'                       'Dのみの場合
...
''Cng End 2012/05/31 SMPK H.Ohkubo
                       ...
                       'Add Start 2012/05/31 SMPK H_Ohkubo
                       Dim ErrInfo      As ERROR_INFOMATION
                       Dim tCOSF3P      As C_COSF3
                       
                       tCOSF3P.HSXCOSF3PK = .typ_si(UpDo).HSXCOSF3PK
                       tCOSF3P.COSF3PTNJSK = typ_Osfz.COSF3PTNJSK
                       
                       If bJudg = True Then
                            If CrystalCOSF3PJudg(tCOSF3P, ErrInfo) <> FUNCTION_RETURN_SUCCESS Then
                                '判定NG
                                bJudg = False
                            ElseIf tCOSF3P.JudgCOSF3 = False Then
                                '判定NG
                                bJudg = False
                            End If
                       End If
...
Public Function CuDecoDataSet_C(pJudgSC_B() As Judg_Spec_Cry, ptyp_b As typ_AllTypesB, UpDo As Integer, DispLineCount As Integer, typTmList() As typ_TBCMB005, sHinban12 As String, sPGID As String, Optional pblnFlag As Boolean = False) As Boolean
''' 2008/10/01 L/DL,OSF判定ﾛｼﾞｯｸ追加 UPD By Systech End
'Chg End 2012/05/29 SMPK H.Ohkubo CLESTA評価レベル判定対応

'注意）CrystalCJudg⇔CuDecoDataSet_Cを必ず両方修正すること。
...
'            If GetCOSF3ID(strJDGEIDC, strXTALC1) <> FUNCTION_RETURN_SUCCESS Then
            'C－OSF3判定IDを獲得する
            If GetJDGEID(sPGID, 1, tDmyHinDt, Trim(left(sHinban12, 8)), strXTALC1, strJDGEIDC) <> FUNCTION_RETURN_SUCCESS Then
'Chg End 2012/05/29 SMPK H.Ohkubo CLESTA評価レベル判定対応
                iErrFlg = 11
            Else
                If Trim(strJDGEIDC) = "" Then
...
Public Function CuDecoDataSet_CJ(pJudgSC_B() As Judg_Spec_Cry, ptyp_b As typ_AllTypesB, UpDo As Integer, DispLineCount As Integer, typTmList() As typ_TBCMB005, sHinban12 As String, sPGID As String, Optional pblnFlag As Boolean = False) As Boolean
''' 2008/10/01 L/DL,OSF判定ﾛｼﾞｯｸ追加 UPD By Systech End
'Chg End 2012/05/29 SMPK H.Ohkubo CLESTA評価レベル判定対応

'注意）CrystalCJJudg⇔CuDecoDataSet_CJを必ず両方修正すること。
...
'Add Start 2012/05/22 SMPK H.Ohkubo CLESTA評価レベル判定対応
                        ' CJ Disk半径の判定(下限値チェック)
                        If bJudg Then
                            If (strPtnJsk = CNST_JSK_PTN_Disk) Or (strPtnJsk = CNST_JSK_PTN_DiskRing) Then
                                If (typ_Ret_CuDeco.CJALLMINDIC5 = -1) Or (typ_Ret_CuDeco.CJALLMINDIC5 > 150) Then
                                    bJudg = False
                                    intNG_Num = 61
                                ElseIf (.typ_zi.CuCJ(UpDo).CJDISKJSK = -1) Or (.typ_zi.CuCJ(UpDo).CJDISKJSK > 150) Then
                                    bJudg = False
                                    intNG_Num = 62
                                ElseIf (typ_Ret_CuDeco.CJALLMINDIC5 > .typ_zi.CuCJ(UpDo).CJDISKJSK) Then
                                    bJudg = False
                                    intNG_Num = 71
                                End If
                            End If
...
Public Function CuDecoDataSet_CJLT(pJudgSC_B() As Judg_Spec_Cry, ptyp_b As typ_AllTypesB, UpDo As Integer, DispLineCount As Integer, typTmList() As typ_TBCMB005, sHinban12 As String, sPGID As String, Optional pblnFlag As Boolean = False) As Boolean
''' 2008/10/01 L/DL,OSF判定ﾛｼﾞｯｸ追加 UPD By Systech End
'Chg End 2012/05/29 SMPK H.Ohkubo CLESTA評価レベル判定対応

'注意）CrystalCJLTJudg⇔CuDecoDataSet_CJLTを必ず両方修正すること。
...
Public Function CuDecoDataSet_CJ2(pJudgSC_B() As Judg_Spec_Cry, ptyp_b As typ_AllTypesB, UpDo As Integer, DispLineCount As Integer, typTmList() As typ_TBCMB005, sHinban12 As String, sPGID As String, Optional pblnFlag As Boolean = False) As Boolean
''' 2008/10/01 L/DL,OSF判定ﾛｼﾞｯｸ追加 UPD By Systech End
'Chg End 2012/05/29 SMPK H.Ohkubo CLESTA評価レベル判定対応

'注意）CrystalCJ2Judg⇔CuDecoDataSet_CJ2を必ず両方修正すること。
