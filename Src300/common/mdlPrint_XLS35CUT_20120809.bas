...
'<<<<< マルチ品番対応改修　2009/12/08　SSS.Marushita
    ...
    sSQL = sSQL & ",NVL(CS.CRYRESCJCS , ' ') CRYRESCJCS"    '実績LG(CJ)
    sSQL = sSQL & ",NVL(CS.CRYRESCJLTCS , ' ') CRYRESCJLTCS" '実績LG(CJLT)
    sSQL = sSQL & ",NVL(CS.CRYRESCJ2CS , ' ') CRYRESCJ2CS"  '実績LG(CJ2)
    'Add End   2011/02/02 SMPK Miyata
'>>>>> 代表サンプルIDの取得対応　2009/01/26　Marushita
    sSQL = sSQL & ",NVL(CS.REPSMPLIDCS , 0) REPSMPLIDCS"    '代表サンプルID
'<<<<< 代表サンプルIDの取得対応　2009/01/26　Marushita
    
    'C,CJ,CJ2対応 2010/11/02 SETsw kubota
    sSQL = sSQL & ",NVL(E036.HSXCHWYS , ' ') HSXCHWYS"      '品SXL/C保証方法＿処
    sSQL = sSQL & ",NVL(E036.HSXCJHWYS , ' ') HSXCJHWYS"    '品SXL/CJ保証方法＿処
    sSQL = sSQL & ",NVL(E036.HSXCJ2HWYS , ' ') HSXCJ2HWYS"  '品SXL/CJ2保証方法＿処
    
    sSQL = sSQL & "  FROM XSDCS CS"
    sSQL = sSQL & "     , XSDC2 C2"
    sSQL = sSQL & "     , TBCME036 E036"    '2010/11/02追加 SETsw kubota
    sSQL = sSQL & " WHERE C2.CRYNUMC2 = '" & sXtalNo & "'"
    sSQL = sSQL & "   AND C2.XTALC2 = CS.XTALCS"
    sSQL = sSQL & "   AND CS.LIVKCS <> '1'"
    sSQL = sSQL & "   AND CS.HINBCS = E036.HINBAN(+)"
    sSQL = sSQL & "   AND CS.REVNUMCS = E036.MNOREVNO(+)"
    sSQL = sSQL & "   AND CS.FACTORYCS = E036.FACTORY(+)"
    sSQL = sSQL & "   AND CS.OPECS = E036.OPECOND(+)"
                    ...
                    If iBlock = 1 Then
                        ...
                        And objDynaData("CRYRESL4CS").Value = "0" Then
                            ...
                            If objDynaData("TBKBNCS").Value = "T" Then
                                ...
                                'Del Start 2011/02/04 SMPK Miyata
                                ''>>>>> C,CJ,CJ2対応 2010/11/02 SETsw kubota ----------
                                ''Chg  2011/02/02 SMPK Miyata 品SXL/C保証方法＿処->指示、実績
                                ...
                                'End If
                                ''<<<<< C,CJ,CJ2対応 2010/11/02 SETsw kubota ----------
                                'Del End   2011/02/04 SMPK Miyata
...
'<<<<< 色識別管理対応追加 2011/12/01 SET.Abe
            ...
            xlSheet.Cells(lBaseRow + 39, 6).Value = "'" & .sMaisu            'サンプル枚数合計
...
'Add End 2011/03/08 SMPK Nakamura FRSシステム化対応
                ...
                Next lSmplCnt
                '>>>>> サンプル位置トップボトム明示対応  2009/11/12　SSS.Marushita
                xlSheet.Cells(lBaseRow + 37, 8).Value = "'" & mtPrintInfo.tMeisai(lCnt - 1).sSmpNoB       'サンプル№BOT
                xlSheet.Cells(lBaseRow + 38, 8).Value = "'" & mtPrintInfo.tMeisai(lCnt - 1).sSmpNoT       'サンプル№TOP
                'xlSheet.Cells(lBaseRow + 36, 8).Value = "'" & mtPrintInfo.tMeisai(lCnt - 1).sSmpNo       'サンプル№
                '<<<<< サンプル位置トップボトム明示対応  2009/11/12　SSS.Marushita
                xlSheet.Cells(lBaseRow + 39, 8).Value = "'" & mtPrintInfo.tMeisai(lCnt - 1).sMaisu       'サンプル指示(抵抗)
...
'                    Call OvalWrite(xlSheet, lBaseRow + 39 + lSmplCnt * 5, 8)
                    If mtPrintInfo.tMeisai(lCnt - 1).sSmplPic(lSmplCnt, 0) = "X" Then
                        ...
                        Call OvalWrite(xlSheet, 41, 4)
                        xlSheet.Cells(42, 4).Value = mtPrintInfo.tMeisai(lCnt - 1).sSmplPic(lSmplCnt, 0)
                        lEditCntMax = 4     '通常の位置にX線を編集しない分、一枚多くループ
                    Else
                        lEditCntMax = 3
                        Call OvalWrite(xlSheet, lBaseRow + 41 + lSmplCnt * 5, 8)
                    End If
'<<<<< X線測定対応 2009/09/02 SETsw kubota ------------------
                        ...
                        If mtPrintInfo.tMeisai(lCnt - 1).sSmplPic(lSmplCnt, lPicCnt) <> PIC_DUMMY Then
                            ...
                            Case 0  '左上
                                If mtPrintInfo.tMeisai(lCnt - 1).sSmplPic(lSmplCnt, 0) <> "X" Then   'X線は二ページ目以降○表示なし
                                    xlSheet.Cells(lBaseRow + 42 + lSmplCnt * 5, 8).Value = mtPrintInfo.tMeisai(lCnt - 1).sSmplPic(lSmplCnt, lPicCnt)
                                    
                                    '右上が空でなければ右と下に罫線を引く
                                    If mtPrintInfo.tMeisai(lCnt - 1).sSmplPic(lSmplCnt, 1) <> "" Then
                                        sStrCell = ConvXlsNumToA(8) & CStr(lBaseRow + 41 + lSmplCnt * 5)
                                        sEndCell = ConvXlsNumToA(8) & CStr(lBaseRow + 42 + lSmplCnt * 5)
                                    ...
                                    If mtPrintInfo.tMeisai(lCnt - 1).sSmplPic(lSmplCnt, 2) <> "" Then
                                        sStrCell = ConvXlsNumToA(8) & CStr(lBaseRow + 42 + lSmplCnt * 5)
                                        sEndCell = ConvXlsNumToA(9) & CStr(lBaseRow + 42 + lSmplCnt * 5)
                                        With xlSheet.Range(sStrCell, sEndCell)
                                            .Borders(xlEdgeBottom).lineStyle = xlContinuous '通常線
                                            .Borders(xlEdgeBottom).Weight = xlThin
                                        End With
                                ...
                                End If
                            Case 1
                                xlSheet.Cells(lBaseRow + 42 + lSmplCnt * 5, 9).Value = mtPrintInfo.tMeisai(lCnt - 1).sSmplPic(lSmplCnt, lPicCnt)
                            Case 2
                                ...
                                If mtPrintInfo.tMeisai(lCnt - 1).sSmplPic(lSmplCnt, 3) <> "" Then
                                    ...
                                    sEndCell = ConvXlsNumToA(8) & CStr(lBaseRow + 44 + lSmplCnt * 5)
                                    With xlSheet.Range(sStrCell, sEndCell)
                                        .Borders(xlEdgeRight).lineStyle = xlContinuous  '通常線
                                        .Borders(xlEdgeRight).Weight = xlThin
                                    End With
...
'>>>>> サンプル№の表示　2009/01/26　Marushita
            '>>>>> サンプル位置トップボトム明示対応  2009/11/12　SSS.Marushita
            xlSheet.Cells(lBaseRow + 37, lBaseCol).Value = "'" & .sSmpNoB      'サンプル№BOT
            xlSheet.Cells(lBaseRow + 38, lBaseCol).Value = "'" & .sSmpNoT      'サンプル№TOP
            ''''xlSheet.Cells(lBaseRow + 36, lBaseCol).Value = "'" & .sSmpNo       'サンプル№
            '<<<<< サンプル位置トップボトム明示対応  2009/11/12　SSS.Marushita
            xlSheet.Cells(lBaseRow + 39, lBaseCol).Value = "'" & .sMaisu       'サンプル指示(枚数)
...
''<<<<< サンプル№表示対応　2009/01/26　Marushita
                If .sSmplPic(lSmplCnt, 0) = "X" Then
                    'X線の場合、一ページ目の合計の下に○表示
                    Call OvalWrite(xlSheet, 41, 4)
                    xlSheet.Cells(42, 4).Value = .sSmplPic(lSmplCnt, 0)
                    lEditCntMax = 4     '通常の位置にX線を編集しない分、一枚多くループ
                Else
                    Call OvalWrite(xlSheet, lBaseRow + 41 + lSmplCnt * 5, lBaseCol)
                    lEditCntMax = 3
                End If
'<<<<< X線測定対応 2009/09/02 SETsw kubota ------------------
...
'>>>>> X線測定対応 2009/09/02 SETsw kubota ------------------
                            If .sSmplPic(lSmplCnt, 0) <> "X" Then
'<<<<< X線測定対応 2009/09/02 SETsw kubota ------------------
                                xlSheet.Cells(lBaseRow + 42 + lSmplCnt * 5, lBaseCol).Value = .sSmplPic(lSmplCnt, lPicCnt)
                                
                                '右上が空でなければ右と下に罫線を引く
                                If .sSmplPic(lSmplCnt, 1) <> "" Then
                                    sStrCell = ConvXlsNumToA(lBaseCol) & CStr(lBaseRow + 41 + lSmplCnt * 5)
                                    sEndCell = ConvXlsNumToA(lBaseCol) & CStr(lBaseRow + 42 + lSmplCnt * 5)
                                    With xlSheet.Range(sStrCell, sEndCell)
                                        .Borders(xlEdgeRight).lineStyle = xlContinuous  '通常線
                                        .Borders(xlEdgeRight).Weight = xlThin
                                    End With
                                ...
                                If .sSmplPic(lSmplCnt, 2) <> "" Then
                                    sStrCell = ConvXlsNumToA(lBaseCol) & CStr(lBaseRow + 42 + lSmplCnt * 5)
                                    sEndCell = ConvXlsNumToA(lBaseCol + 1) & CStr(lBaseRow + 42 + lSmplCnt * 5)
                                    With xlSheet.Range(sStrCell, sEndCell)
                                        .Borders(xlEdgeBottom).lineStyle = xlContinuous '通常線
                                        .Borders(xlEdgeBottom).Weight = xlThin
                                    End With
...
'<<<<< X線測定対応 2009/09/02 SETsw kubota ------------------
                        ...
                        Case 1
'>>>>> サンプル№表示対応　2009/01/26　Marushita
                            xlSheet.Cells(lBaseRow + 42 + lSmplCnt * 5, lBaseCol + 1).Value = .sSmplPic(lSmplCnt, lPicCnt)
...
'>>>>> サンプル№表示対応　2009/01/26　Marushita
                                ...
                                sEndCell = ConvXlsNumToA(lBaseCol) & CStr(lBaseRow + 44 + lSmplCnt * 5)
