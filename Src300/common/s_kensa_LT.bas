...
'          :bHaneiCalc    ,I   ,Boolean      ,抵抗が反映の時に計算をするかのフラグ True - 計算する False - しない 2011/08/31
'          :戻り値        ,O  ,FUNCTION_RETURN  ,抽出の成否
'説明      :
'備考      : １０Ω換算式の算出方法
'               Ａ＝ライフタイム測定結果
...
Public Function GetKansanchi(tblCrySmpMan As typ_XSDCS, sKekka As String, sIncVal As String, _
        sCutVal As String, sSetVal As String, sJiteiko As String, sKansanchi As String _
      , Optional ByVal bHaneiCalc As Boolean = False) As Integer
    Dim sql As String       'SQL全体
    Dim rs As OraDynaset    'RecordSet
...
'>>>>> 反映サンプルでの10Ω換算対応 2011/08/31 SETsw kubota -----------
'    sql = sql & " AND   SMPKBN='" & tblCrySmpMan.SMPKBNCS & "'"
    If bHaneiCalc = True And (tblCrySmpMan.CRYINDRSCS = "1" Or tblCrySmpMan.CRYINDRSCS = "2") Then
        'サンプルIDを条件(抵抗が反映の場合も計算)
        sql = sql & " AND   SMPLNO='" & tblCrySmpMan.CRYSMPLIDRSCS & "'"
    Else
        'サンプル区分を条件(既存の条件)
        sql = sql & " AND   SMPKBN='" & tblCrySmpMan.SMPKBNCS & "'"
    End If
