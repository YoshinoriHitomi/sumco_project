...
Public Type typ_chk1_13
    ...
    'Cng Start 2012/07/09 Y.Hitomi
    MLTJDG(2) As String
    'MLTJDG() As String
    'Cng End 2012/07/09 Y.Hitomi
End Type
...
'エッヂ補正 品番振替チェック 2011/11/09 Marushita
Public Type typ_chk1_17
    hinban As tFullHinban
    HWFRSPOH  As String           'WF比抵抗　測定位置＿方
End Type

Public tbl_chk1_17(1) As typ_chk1_17
'エッヂ補正 マルチブロック組合せチェック 2011/11/09 Marushita
Public Type typ_chk1_18
    hinban As tFullHinban
    HWFRSPOH  As String           'WF比抵抗　測定位置＿方
End Type
Public tbl_chk1_18(1) As typ_chk1_18
Public Const SAMPLE_NOHOSEI = "C"       '「１／２サンプル補正なし」コード（"C"） 2011/12/05 Marushita

'Add Start 2011/12/08 Y.Hitomi 品種チェック対応
Public Type typ_chk1_19
    MCNO1       As String * 1       '品種
    MCNO2       As String * 1       '引上げ速度
    MCNO3       As String * 1       'HZタイプ
End Type
Public tbl_chk1_19(1) As typ_chk1_19
'Add End 2011/12/08 Y.Hitomi
...
Public Type typ_chk2_6
    FRSFLG          As String       'FRSフラグ
    QCLEVELCODE()   As String       '品質レベルコード
    FRSTOTALJUDGE() As String       'FRS総合判定
    JUDGE           As String       '判定
    JUDGE1          As String       'CLESTA判定
    JUDGE2          As String       'CLESTA-Z判定
    STSFLG          As String       '状態フラグ     ADD 2012/04/20 SMPK H.Ohkubo FRS判定対応
End Type
Public tbl_chk2_6 As typ_chk2_6
'Add End 2011/10/12 Y.Hitomi
'Add Start 2011/12/08 Y.Hitomi
Public Type typ_chk2_7
    sPGIDCHK           As String       'PGIDチェック結果
End Type
Public tbl_chk2_7 As typ_chk2_7
'Add End 2011/12/08 Y.Hitomi


'2012/01/10 Add START DCS)Shoryuji 結晶ボトム側払出規制追加対応
Public Type typ_chk2_8                  'テイル規制チェック結果構造体
    TSPMINRES   As Double               'テイル速度MIN実績
    TSTPWRRES   As Double               'テイル開始パワー実績
    TSPMINJUDGE As String               'テイル速度判定結果
    TSTPWRJUDGE As String               'テイル開始パワー判定結果
End Type
Public tbl_chk2_8 As typ_chk2_8    'テイル規制チェック判定結果
'2012/01/10 Add E_N_D DCS)Shoryuji
...
'          :iNew_StartPos   ,I  ,Integer        :振替先品番開始位置 2012/01/05 Add DCS)Shoryuji
'          :iNew_EndPos     ,I  ,Integer        :振替先品番終了位置 2012/01/05 Add DCS)Shoryuji
'          :bJudgeNG_Through,I  ,Boolean        :振替NGでも実行可フラグ
...
''<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
'2012/01/05 Update START DCS)Shoryuji 結晶ボトム側払出規制追加対応
'Public Function funChkFurikaeShiyou(sProccd As String, sKeyID As String, tOld_Hinban As tFullHinban, tNew_Hinban As tFullHinban, _
'                                    iErr_Code As Integer, sErr_Msg As String, _
'                                    typ_b As typ_AllTypesB, typ_CType As typ_AllTypesC, _
'                                    iSmpGetFlg As Integer, _
...
Public Function funChkFurikaeShiyou(sProccd As String, sKeyID As String, tOld_Hinban As tFullHinban, tNew_Hinban As tFullHinban, _
                                    ...
                                    Optional iCC10 As Integer = 0, _
                                    Optional sPlshMeth As String = "", _
                                    Optional iNew_StartPos As Integer = 0, _
                                    Optional iNew_EndPos As Integer = 0, _
                                    Optional b2_8JudgeNG_Through As Boolean = False _
                                  ) As Integer
'2012/01/05 Update E_N_D DCS)Shoryuji
...
'Add Start 2011/11/09 Marushita エッヂ補正品番振替チェック追加対応
    '1-17 エッヂ補正仕様チェック
    If Mid(sResult, 17, 1) = "1" Then
        If iMultiFlg = 0 Then
            If iELCs_Flg = 0 Or iELCs_Flg = 2 Then
                RET = funChkFurikae1_17(sProccd, sKeyID, tOld_Hinban, tNew_Hinban, iErr_Code, sErr_Msg)
                If RET <> 0 Then
                    funChkFurikaeShiyou = RET
                    If RET > 0 Then GoTo Apl_Exit
                    GoTo Apl_Error
        ...
        End If
    End If
'Add End 2011/11/09 Marushita エッヂ補正品番振替チェック追加対応
'Add Start 2011/12/08 Y.Hitomi 品種チェック対応
    '1-19 品種、HZパーツチェック
    If Mid(sResult, 19, 1) = "1" Then
        If iMultiFlg = 0 Then
            If iELCs_Flg = 0 Or iELCs_Flg = 2 Then
                RET = funChkFurikae1_19(sProccd, sKeyID, tOld_Hinban, tNew_Hinban, iErr_Code, sErr_Msg)
                If RET <> 0 Then
                    funChkFurikaeShiyou = RET
                    If RET > 0 Then GoTo Apl_Exit
                    GoTo Apl_Error
        ...
        End If
    End If
'Add End 2011/12/08 Y.Hitomi 品種チェック対応
    '2-1 結晶評価実績チェック
'    If Mid(sResult, 11, 1) = "1" Then
...
'    If Mid(sResult, 14, 1) = "1" Then
    ...
    'Add Start 2011/10/12 Y.Hitomi
    '2-6 FRSブロック評価チェック
    If Mid(sResult2, 6, 1) = "1" Then       'FEの6桁目
            RET = funChkFurikae2_6(sProccd, sKeyID, tOld_Hinban, tNew_Hinban, iHcnt, iErr_Code, sErr_Msg)
            If RET <> 0 Then
                funChkFurikaeShiyou = RET
                '判定(CC600)は抜けない
                If sProccd = "CC600" Or sProccd = "CW750" Then
...
'    If Mid(sResult, 22, 1) = "1" Then
    If Mid(sResult3, 2, 1) = "1" Then       'FCの22桁目→FFの2桁目 2011/04/07 SETsw kubota
        If iMultiFlg = 0 Then ''<<複数品番判定対応20060502 SMP桜井　ブロック品番保障0,2のMultiBlockの真ん中はしない
            If iELCs_Flg = 0 Or iELCs_Flg = 2 Then  ''<<複数品番判定対応20060502 SMP桜井　振替元品番チェック振替元以外の品番はしない
                'Cng Start 2011/11/29 Y.Hitomi
                RET = funChkFurikae3_2(sProccd, tOld_Hinban, tNew_Hinban, iErr_Code, sErr_Msg)
                'RET = funChkFurikae3_2(tOld_Hinban, tNew_Hinban, iErr_Code, sErr_Msg)
                'Cng End 2011/11/29 Y.Hitomi
                If RET <> 0 Then
                    funChkFurikaeShiyou = RET
                    If RET > 0 Then GoTo Apl_Exit
                    GoTo Apl_Error
...
'2012/01/04 Add START DCS)Shoryuji 結晶ボトム側払出規制追加対応
'テイル規制チェックの判定NGでも実行可に対応する為、
'このチェックを判定処理の最終に配置。
    '2-8 テイル規制チェック - 結晶ボトム側払出規制対応
    If Mid(sResult2, 8, 1) = "1" Then       'FEの8桁目
            RET = funChkFurikae2_8(sProccd, sKeyID, tOld_Hinban, tNew_Hinban, iNew_StartPos, iNew_EndPos, iHcnt, iErr_Code, sErr_Msg)
            '判定NGでも実行可に対応
            If b2_8JudgeNG_Through Then
                If RET < 0 Then
                    funChkFurikaeShiyou = RET
                    GoTo Apl_Error
                End If
            ...
            End If
    End If
'2012/01/04 Add E_N_D DCS)Shoryuji
...
'    End If
    ...
    '振替指示データ取得
    sResult = ""
    'Cng Start 2011/10/26 Y.Hitomi
    RET = funCodeDBGet("SB", "FD", sProccd, 0, " ", sResult)
    'RET = funCodeDBGet("SB", "FG", sProccd, 0, " ", sResult)
    'Cng End 2011/10/26 Y.Hitomi
    If RET <> 0 Then
        funChkMultiShiyou = -2
        GoTo Apl_Error
    End If
    ...
    If Mid(sResult, 16, 1) = "1" Then
        ...
        End If
    End If
'Add Start 2011/11/09 Marushita エッヂ補正マルチブロック組合せチェック追加対応
    '1-18 エッヂ補正マルチブロック組合せチェック
    If Mid(sResult, 18, 1) = "1" Then
        RET = funChkFurikae1_18(sProccd, sKeyID, tTop_Hinban, tBtm_Hinban, iErr_Code, sErr_Msg)
        If RET <> 0 Then
            funChkMultiShiyou = RET
            If RET > 0 Then GoTo Apl_Exit
            GoTo Apl_Error
        End If
    End If
'Add End 2011/11/09 Marushita エッヂ補正マルチブロック組合せチェック追加対応

'Add Start 2012/06/19 Y.Hitomi
    '1-20 CLESTAマルチブロック組合せチェック
    If Mid(sResult, 20, 1) = "1" Then
        'Top品番→Bot品番への振り替え可否チェック
        RET = funChkFurikae2_2(sProccd, sKeyID, tTop_Hinban, tBtm_Hinban, iErr_Code, sErr_Msg)
        If RET <> 0 Then
            funChkMultiShiyou = RET
            If RET > 0 Then GoTo Apl_Exit
            GoTo Apl_Error
        End If
        'Bot品番→Top品番への振り替え可否チェック
        RET = funChkFurikae2_2(sProccd, sKeyID, tBtm_Hinban, tTop_Hinban, iErr_Code, sErr_Msg)
        If RET <> 0 Then
            funChkMultiShiyou = RET
            If RET > 0 Then GoTo Apl_Exit
            GoTo Apl_Error
...
'Del Start 2011/12/08 Y.Hitomi チェック1-19に変更
'    '品種のチェック
'    sErr_Msg = "1-2 品種ﾁｪｯｸ"
'    sResult = ""
...
Public Function funChkFurikae1_6(sProccd As String, sBlockId As String, tOld_Hinban As tFullHinban, tNew_Hinban As tFullHinban, _
    ...
    If tbl_chk1_6(1).HWFNP1AR = 2 And tbl_chk1_6(1).HWFNP1MAX <= 17 Or _
       tbl_chk1_6(1).HWFNP2AR = 10 And tbl_chk1_6(1).HWFNP2MAX <= 50 Then
        W_NEW_FLG = 1
    End If
'2012/09/11 Del START SETsw)nakagawa ガラス接着規制解除
''    'ガラス接着品のチェック
...
''        GoTo Apl_Exit
''    End If
'2012/09/11 Del END SETsw)nakagawa ガラス接着規制解除
...
'    If tbl_chk1_12(0).MSMPFLG = "0" And tbl_chk1_12(1).MSMPFLG = "1" Then
    If tbl_chk1_12(0).MSMPFLG = "0" And (tbl_chk1_12(1).MSMPFLG = "1" Or tbl_chk1_12(1).MSMPFLG = "3") Then
...
'Del Start 2012/07/03 Y.Hitomi
'        If .RENBAN <= 1 Then GoTo Apl_Exit    'マルチ引上げバッチでも１本目は対象外とします
'Del End 2012/07/03 Y.Hitomi
    ...
    On Error GoTo Apl_down
    sErr_Msg = "1-13 マルチ引上げ適用フラグチェック"
    If tbl_chk1_13(0).MLTHTFLG = "1" Then
        If tbl_chk1_13(1).MLTHTFLG = "0" Then
            funChkFurikae1_13 = 1
            iErr_Code = 1301
            sErr_Msg = "CHECK1-13,マルチ引上げ適用不可品番の為、振替できません。"
...
End Function
'Add End 2011/07/12 SMPK Nakamura 結晶面傾きチェック追加対応
'Add Start 2011/11/09 Marushita
'------------------------------------------------
' 振替先と振替元のエッヂ補正 品番振替チェック
'------------------------------------------------
...
Public Function funChkFurikae1_17(sProccd As String, sBlockId As String, tOld_Hinban As tFullHinban, tNew_Hinban As tFullHinban, _
                                  iErr_Code As Integer, sErr_Msg As String) As Integer
    ...
    '------------------------------------------ 振替元品種仕様データ取得 ------------------------------------------------------
    sErr_Msg = "1-17 振替元品番仕様取得(" & tOld_Hinban.hinban & Format(tOld_Hinban.mnorevno, "00") & tOld_Hinban.factory & tOld_Hinban.opecond & ")"
    'SQL文の作成
    sql = vbNullString
    sql = sql & "SELECT E021.HWFRSPOH " & vbCrLf
    sql = sql & "FROM   TBCME021 E021 " & vbCrLf
    sql = sql & "WHERE  E021.HINBAN    =   '" & tOld_Hinban.hinban & "'  AND " & vbCrLf
    sql = sql & "       E021.MNOREVNO  =    " & tOld_Hinban.mnorevno & " AND " & vbCrLf
    sql = sql & "       E021.FACTORY   =   '" & tOld_Hinban.factory & "' AND " & vbCrLf
    sql = sql & "       E021.OPECOND   =   '" & tOld_Hinban.opecond & "' " & vbCrLf
    ...
    Erase tbl_chk1_17
    With tbl_chk1_17(0)
        If IsNull(rs("HWFRSPOH")) = False Then .HWFRSPOH = rs("HWFRSPOH") Else .HWFRSPOH = " "         'WF比抵抗　測定位置＿方
    End With
    
    Set rs = Nothing
    '------------------------------------------ 振替先品種仕様データ取得 ------------------------------------------------------
    sErr_Msg = "1-17 振替先品番仕様取得(" & tNew_Hinban.hinban & Format(tNew_Hinban.mnorevno, "00") & tNew_Hinban.factory & tNew_Hinban.opecond & ")"
    'SQL文の作成
    sql = vbNullString
    sql = sql & "SELECT E021.HWFRSPOH " & vbCrLf
    sql = sql & "FROM   TBCME021 E021 " & vbCrLf
    ...
    sql = sql & "       E021.MNOREVNO  =    " & tNew_Hinban.mnorevno & " AND " & vbCrLf
    sql = sql & "       E021.FACTORY   =   '" & tNew_Hinban.factory & "' AND " & vbCrLf
    sql = sql & "       E021.OPECOND   =   '" & tNew_Hinban.opecond & "' " & vbCrLf
    ...
    'エッヂ補正 品番振替チェック
    sErr_Msg = "1-17 エッヂ補正振替ﾁｪｯｸ"

    '振替元品番が「１／２サンプル補正なし」か？
    If tbl_chk1_17(0).HWFRSPOH = SAMPLE_NOHOSEI Then
        If tbl_chk1_17(1).HWFRSPOH = SAMPLE_NOHOSEI Then
        Else
            funChkFurikae1_17 = 1
            iErr_Code = 11701
            sErr_Msg = "CHECK1-17,サンプル補正なし品番からは振替できません。"
            gsTbcmy028ErrCode = "00170"
            GoTo Apl_Exit
        End If
    Else
        If tbl_chk1_17(1).HWFRSPOH = SAMPLE_NOHOSEI Then
...
'Cng End   2012/02/23 Y.Hitomi
            iErr_Code = 11702
            sErr_Msg = "CHECK1-17,サンプル補正なし品番へは振替できません。"
            gsTbcmy028ErrCode = "00171"
            GoTo Apl_Exit
        End If
    End If
...
'          :戻り値          ,O  ,Integer      :成否(0:正常終了(振替OK),1:正常終了(振替NG),-2:取得ｴﾗｰ)
'説明      :
'履歴      :2011/11/09 Marushita

Public Function funChkFurikae1_18(sProccd As String, sKeyID As String, _
                                  tTop_Hinban As tFullHinban, tBtm_Hinban As tFullHinban, _
                                  iErr_Code As Integer, sErr_Msg As String) As Integer
    ...
    '------------------------------------------ 先頭品番仕様データ取得 ------------------------------------------------------
    sErr_Msg = "1-18 先頭品番仕様取得(" & tTop_Hinban.hinban & Format(tTop_Hinban.mnorevno, "00") & tTop_Hinban.factory & tTop_Hinban.opecond & ")"
    'SQL文の作成
    sql = vbNullString
    sql = sql & "SELECT E021.HWFRSPOH " & vbCrLf
    sql = sql & "FROM   TBCME021 E021 " & vbCrLf
    sql = sql & "WHERE  E021.HINBAN    =   '" & tTop_Hinban.hinban & "'  AND " & vbCrLf
    sql = sql & "       E021.MNOREVNO  =    " & tTop_Hinban.mnorevno & " AND " & vbCrLf
    sql = sql & "       E021.FACTORY   =   '" & tTop_Hinban.factory & "' AND " & vbCrLf
    sql = sql & "       E021.OPECOND   =   '" & tTop_Hinban.opecond & "' " & vbCrLf
    ...
    '------------------------------------------ 最尾品番仕様データ取得 ------------------------------------------------------
    sErr_Msg = "1-18 最尾品番仕様取得(" & tBtm_Hinban.hinban & Format(tBtm_Hinban.mnorevno, "00") & tBtm_Hinban.factory & tBtm_Hinban.opecond & ")"
    'SQL文の作成
    sql = vbNullString
    sql = sql & "SELECT E021.HWFRSPOH " & vbCrLf
    sql = sql & "FROM   TBCME021 E021 " & vbCrLf
    sql = sql & "WHERE  E021.HINBAN    =   '" & tBtm_Hinban.hinban & "'  AND " & vbCrLf
    sql = sql & "       E021.MNOREVNO  =    " & tBtm_Hinban.mnorevno & " AND " & vbCrLf
    sql = sql & "       E021.FACTORY   =   '" & tBtm_Hinban.factory & "' AND " & vbCrLf
    sql = sql & "       E021.OPECOND   =   '" & tBtm_Hinban.opecond & "' " & vbCrLf
    ...
    '先頭品番が「１／２サンプル補正なし」か？
    If tbl_chk1_18(0).HWFRSPOH = SAMPLE_NOHOSEI Then
        If tbl_chk1_18(1).HWFRSPOH = SAMPLE_NOHOSEI Then
        Else
            funChkFurikae1_18 = 1
            iErr_Code = 11801
            sErr_Msg = "CHECK1-18,ｻﾝﾌﾟﾙ補正なし仕様が混在する為、組合せできません。"
            gsTbcmy028ErrCode = "00180"
            GoTo Apl_Exit
        End If
    Else
        If tbl_chk1_18(1).HWFRSPOH = SAMPLE_NOHOSEI Then
...
'          :sPlshMeth       ,I  ,String       :研削方法(加工払出画面) M:MGR(MGRの場合、溝位置方位ロック解除)  2011/10/13 SETsw kubota
'          :戻り値          ,O  ,Integer      :成否(0:正常終了(振替OK),1:正常終了(振替NG),-2:取得ｴﾗｰ)
'説明      :チェック工程の変更から、チェック1-2から分離し移動
'履歴      :2011/12/08  新規作成    Y.Hitomi

Public Function funChkFurikae1_19(sProccd As String, sBlockId As String, tOld_Hinban As tFullHinban, tNew_Hinban As tFullHinban, _
                                 iErr_Code As Integer, sErr_Msg As String _
                               , Optional ByVal sPlshMeth As String = "" _
                               ) As Integer
    ...
    '------------------------------------------ 振替元品種仕様データ取得 ------------------------------------------------------
    sErr_Msg = "1-19 振替元品番仕様取得(" & tOld_Hinban.hinban & Format(tOld_Hinban.mnorevno, "00") & tOld_Hinban.factory & tOld_Hinban.opecond & ")"
    'SQL文の作成
    sql = vbNullString
    sql = sql & "SELECT " & vbCrLf
    sql = sql & "       SUBSTR(E018.MCNO,1,1) MCNO1,SUBSTR(E018.MCNO,4,1) MCNO2,SUBSTR(E018.MCNO,3,1) MCNO3 " & vbCrLf
    sql = sql & "FROM   TBCME018 E018 " & vbCrLf
    sql = sql & "WHERE  E018.HINBAN    =   '" & tOld_Hinban.hinban & "'  AND " & vbCrLf
    sql = sql & "       E018.MNOREVNO  =    " & tOld_Hinban.mnorevno & " AND " & vbCrLf
    sql = sql & "       E018.FACTORY   =   '" & tOld_Hinban.factory & "' AND " & vbCrLf
    sql = sql & "       E018.OPECOND   =   '" & tOld_Hinban.opecond & "'" & vbCrLf
    ...
    '品種のチェック
    sErr_Msg = "1-19 品種ﾁｪｯｸ"
    sResult = ""
    RET = funCodeDBGet("SB", "HS", tbl_chk1_19(0).MCNO1, 1, tbl_chk1_19(1).MCNO1, sResult)
    If RET <> 0 Then
        sErr_Msg = sErr_Msg & "→元:" & tbl_chk1_19(0).MCNO1 & ", 先:" & tbl_chk1_19(1).MCNO1
        GoTo CodeDBGet_Error
    End If
    If sResult = 0 Then
        funChkFurikae1_19 = 1
        iErr_Code = 1207
        sErr_Msg = "CHECK1-19,品種、振替不可能です。"
        gsTbcmy028ErrCode = "00010"
        GoTo Apl_Exit
    End If
    ...
    sErr_Msg = "1-2 HZﾀｲﾌﾟﾁｪｯｸ"
    sResult = ""
    RET = funCodeDBGet("SB", "HZ", tbl_chk1_19(0).MCNO3, 1, tbl_chk1_19(1).MCNO3, sResult)
    If RET <> 0 Then
        sErr_Msg = sErr_Msg & "→元:" & tbl_chk1_19(0).MCNO3 & ", 先:" & tbl_chk1_19(1).MCNO3
        GoTo CodeDBGet_Error
    End If
    If sResult = 0 Then
        funChkFurikae1_19 = 1
        iErr_Code = 1209
        sErr_Msg = "CHECK1-19,ＨＺタイプ、振替不可能です。"
        gsTbcmy028ErrCode = "00012"
        GoTo Apl_Exit
    End If
...
'履歴      :05/12/28 ooba
'Cng Start 2011/11/29 Y.Hitomi
Public Function funChkFurikae3_2(sProccd As String, tOld_Hinban As tFullHinban, tNew_Hinban As tFullHinban, _
                                 iErr_Code As Integer, sErr_Msg As String) As Integer
...
'Cng End 2011/11/29 Y.Hitomi
    ...
    'Cng Start 2011/11/29 Y.Hitomi
    'Cng 2010/05/31 Y.Hitomi Warpｴﾗｰ緩和対応(10枚）
'    If Not bWarpAllJudg Then
'    If iWarpErrCount > 10 Then
    If iWarpErrCount > 10 And left(sProccd, 4) = "CW74" Then
    'Cng Start 2011/11/29 Y.Hitomi
        funChkFurikae3_2 = 1
        iErr_Code = 3201
...
'--------------- 2008/07/25 INSERT  END   By Systech ---------------
    ...
    'Add Start 2011/11/29 Y.Hitomi
    'Cng Start 2011/12/19 Y.Hitomi
    If Not bWarpAllJudg And left(sProccd, 4) = "CW75" Then
'    If Not bWarpAllJudg And left(sProccd, 4) <> "CW74" Then
    'Cng End 2011/12/19 Y.Hitomi
        funChkFurikae3_2 = 1
        iErr_Code = 3201
        sErr_Msg = "CHECK3-2,Warp実績で不合格の為、振り替えできません。"
        gsTbcmy028ErrCode = "00128"
    End If
    'Add End 2011/11/29 Y.Hitomi
...
Public Function funChkFurikae2_4(sProccd As String, sBlockId As String, tOld_Hinban As tFullHinban, tNew_Hinban As tFullHinban, _
    ...
    Dim strCdni As String
    Dim intSisu As Integer
...
'    '------------------------------------------ CWの場合、ﾌﾞﾛｯｸIDを取得 ------------------------------------------------
    ...
    sql = sql & ",nvl(HSXCDPNI,' ') as HSXCDPNI " & vbCrLf
    ...
    End If
    Set rs = Nothing
    '仕様が14乗以外だったら窒素濃度規格を計算する
    intSisu = 14 - val(strCdni)
    If intSisu <> 0 Then
        If intSisu < 0 Then
            dblNMax = dblNMax * (10 ^ Abs(intSisu))
            dblNMin = dblNMin * (10 ^ Abs(intSisu))
        Else
            dblNMax = dblNMax / (10 ^ Abs(intSisu))
            dblNMin = dblNMin / (10 ^ Abs(intSisu))
        End If
...
'           連続コード１桁目が２以上の場合、エラーとし流動不可とします。
''''''          該当ブロック､シングルがマルチ2本目以降の場合はエラーとします｡ ←2012/06/15削除
...
'    '------------------------------------------ CWの場合、結晶番号を取得 ------------------------------------------------
    ...
    If tbl_chk2_5.MLTHTFLG = "1" Then
        'Cng Start 2012/06/15 Y.Hitomi
        If tbl_chk2_5.SIJICNT > 1 Then
        'If tbl_chk2_5.RENBAN > 1 Then
        'Cng Start 2012/06/15 Y.Hitomi
            funChkFurikae2_5 = 1
            iErr_Code = 2501
            sErr_Msg = "CHECK2-5,マルチ適用不可品番の為、流動できません。" '
...
'説明      :ＦＲＳブロック実績チェック
'          :
'履歴      :2011/10/12  Y.Hitomi    新規追加
Public Function funChkFurikae2_6(sProccd As String, sBlockId As String, tOld_Hinban As tFullHinban, tNew_Hinban As tFullHinban, _
                                 iHinCnt As Integer, iErr_Code As Integer, sErr_Msg As String) As Integer
    Dim sql         As String       'SQL全体
    Dim rs          As OraDynaset   'RecordSet
    Dim s
    ...
    Dim strFrsFlg   As String       'FRS測定対象有無フラグ(0:無/1:有)   Add 2012/07/25 SMPK H.Ohkubo FRS判定対応
    On Error GoTo Apl_down
    
    ReDim tbl_chk2_6.FRSTOTALJUDGE(2)
    ReDim tbl_chk2_6.QCLEVELCODE(2)
    '戻り値初期化
    funChkFurikae2_6 = 0
        
    '-------------------------------- 振替先ブロック評価仕様データ取得 ------------------------------------------------------
    sErr_Msg = "2-6 品番仕様取得(" & tNew_Hinban.hinban & Format(tNew_Hinban.mnorevno, "00") & tNew_Hinban.factory & tNew_Hinban.opecond & ")"
    'SQL文の作成
    sql = vbNullString
    sql = sql & "SELECT NVL(E036.QCLEVELCODE,' ') QCLEVELCODE " & vbCrLf
    'Add Start 2012/07/25 SMPK H.Ohkubo FRS判定対応
    sql = sql & ", NVL(E036.FRSFLG,' ') FRSFLG " & vbCrLf
    'Add End 2012/07/25 SMPK H.Ohkubo FRS判定対応
    sql = sql & "FROM   TBCME036 E036 " & vbCrLf
    sql = sql & "WHERE  E036.HINBAN    =   '" & tNew_Hinban.hinban & "'  AND " & vbCrLf
    sql = sql & "       E036.MNOREVNO  =    " & tNew_Hinban.mnorevno & " AND " & vbCrLf
    sql = sql & "       E036.FACTORY   =   '" & tNew_Hinban.factory & "' AND " & vbCrLf
    ...
    '
    If Trim(rs("QCLEVELCODE")) <> "" Then
        tbl_chk2_6.QCLEVELCODE(iHinCnt) = rs("QCLEVELCODE")
    Else
        tbl_chk2_6.QCLEVELCODE(iHinCnt) = "00"
    End If
...
'    '------------------------------------------ CWの場合、ブロック番号を取得 ------------------------------------------------
    ...
    If IsNull(rs("CRYNUMCA")) = False Then sCryNum = rs("CRYNUMCA") Else sCryNum = " "
    
    Set rs = Nothing
    
    '------------------------------------------ 判定データ取得(XSDC_1) ------------------------------------------
    sql = "SELECT NVL(CRYRESOIFRSCS1,' ') as FRSFLG,NVL(CRYRESOIFRSCS1,' ') as JUDGE  " & vbCrLf
    sql = sql & ",NVL(JUDGE1FRSCS1,' ') as JUDGE1,NVL(JUDGE2FRSCS1,' ') as JUDGE2 " & vbCrLf
    'Add Start 2012/04/20 SMPK H.Ohkubo FRS判定対応
    sql = sql & ",NVL(CRYINDOIFRSCS1,' ') as STSFLG " & vbCrLf
    'Add End 2012/04/20 SMPK H.Ohkubo FRS判定対応
    sql = sql & "FROM XSDCS_1 " & vbCrLf
    sql = sql & "WHERE CRYNUMCS1 = '" & sCryNum & "' "
    ...
    tbl_chk2_6.FRSTOTALJUDGE(iHinCnt) = "OK"

    sErr_Msg = "2-6 ブロック評価チェック"
    
    'Add Start 2012/06/12 Y.Hitomi
    If tbl_chk2_6.FRSFLG = "0" Then
        GoTo Apl_Exit
    End If
    'Add End 2012/06/12 Y.Hitomi
    ...
    If tbl_chk2_6.QCLEVELCODE(iHinCnt) = "12" And tbl_chk2_6.JUDGE1 <> "1" Then
        tbl_chk2_6.FRSTOTALJUDGE(iHinCnt) = "NG"
        funChkFurikae2_6 = 1
        iErr_Code = 2601
        sErr_Msg = "CHECK2-6,FRS CLESTA判定NGの為、振り替えできません。" '
        gsTbcmy028ErrCode = "02601"
        GoTo Apl_Exit
    ElseIf tbl_chk2_6.QCLEVELCODE(iHinCnt) = "13" And tbl_chk2_6.JUDGE2 <> "1" Then
        tbl_chk2_6.FRSTOTALJUDGE(iHinCnt) = "NG"
        funChkFurikae2_6 = 1
        iErr_Code = 2602
        sErr_Msg = "CHECK2-6,FRS CLESTA-Z判定NGの為、振り替えできません。" '
        gsTbcmy028ErrCode = "02602"
        GoTo Apl_Exit
    'Add Start 2012/04/20 SMPK H.Ohkubo FRS判定対応
    ElseIf tbl_chk2_6.STSFLG <> "0" And tbl_chk2_6.JUDGE <> "1" Then
        tbl_chk2_6.FRSTOTALJUDGE(iHinCnt) = "NG"
        funChkFurikae2_6 = 1
        iErr_Code = 2603
        sErr_Msg = "CHECK2-6,FRS判定NGの為、振り替えできません。" '
        gsTbcmy028ErrCode = "02603"
        GoTo Apl_Exit
    'Add End 2012/04/20 SMPK H.Ohkubo FRS判定対応
    End If
...
'説明      :製作条件PGIDチェック
'          :
'履歴      :2011/12/08  Y.Hitomi    新規追加
Public Function funChkFurikae2_7(sProccd As String, sBlockId As String, tOld_Hinban As tFullHinban, tNew_Hinban As tFullHinban, _
                                 iHinCnt As Integer, iErr_Code As Integer, sErr_Msg As String) As Integer
    Dim sql         As String       'SQL全体
    Dim rs          As OraDynaset   'RecordSet
    Dim s
    ...
    Dim i           As Integer
    Dim sPGID       As String
    Dim iPGIDCNT    As String
    
    On Error GoTo Apl_down
    
    '戻り値初期化
    funChkFurikae2_7 = 0
    tbl_chk2_7.sPGIDCHK = "OK"
    '------------------ 振替元結晶のPGID取得 ------------------------------------------------------
    sErr_Msg = "2-7 品番仕様取得(" & tNew_Hinban.hinban & Format(tNew_Hinban.mnorevno, "00") & tNew_Hinban.factory & tNew_Hinban.opecond & ")"
    
    'SQL文の作成
    sql = vbNullString
    sql = sql & "SELECT NVL(H001.PGID,' ') PGID " & vbCrLf
    sql = sql & "FROM   TBCMH001 H001,XSDC1 C1,XSDC2 C2 " & vbCrLf
    sql = sql & "WHERE " & vbCrLf
    sql = sql & "       H001.UPINDNO    =   C1.HISIJIC1     AND " & vbCrLf
    sql = sql & "       C2.XTALC2       =   C1.XTALC1       AND " & vbCrLf
    sql = sql & "       C2.CRYNUMC2       =   '" & sBlockId & "'  " & vbCrLf
    ...
    '------------------ 振替先ブロックの製作条件/使用PGID紐付け情報取得 -------------------
    sErr_Msg = "2-7 品番仕様取得(" & tNew_Hinban.hinban & Format(tNew_Hinban.mnorevno, "00") & tNew_Hinban.factory & tNew_Hinban.opecond & ")"
    'SQL文の作成
    sql = vbNullString
    sql = sql & "SELECT count(*) as PGIDCNT " & vbCrLf
'>>>>> TBCME030をTBCME036に変更 2012/08/27 SETsw kubota ----------------------------------
'    sql = sql & "FROM   TBCME030 E030,TBCMB013 B013 " & vbCrLf
'    sql = sql & "WHERE  trim(E030.MCNO) =   trim(B013.MKCONDNO) AND" & vbCrLf
'    sql = sql & "       E030.HINBAN     =   '" & tNew_Hinban.hinban & "'  AND " & vbCrLf
'    sql = sql & "       B013.PGIDNO     =   '" & Trim(sPgid) & "'" & vbCrLf
    sql = sql & "FROM   TBCME036 E036,TBCMB013 B013 " & vbCrLf
    sql = sql & "WHERE  trim(E036.MCNO) =   trim(B013.MKCONDNO)" & vbCrLf
    sql = sql & "  AND  E036.HINBAN     =   '" & tNew_Hinban.hinban & "'" & vbCrLf
    sql = sql & "  AND  E036.MNOREVNO   =   '" & tNew_Hinban.mnorevno & "'" & vbCrLf
    sql = sql & "  AND  E036.FACTORY    =   '" & tNew_Hinban.factory & "'" & vbCrLf
    sql = sql & "  AND  E036.OPECOND    =   '" & tNew_Hinban.opecond & "'" & vbCrLf
    sql = sql & "  AND  B013.PGIDNO     =   '" & Trim(sPGID) & "'" & vbCrLf
'<<<<< TBCME030をTBCME036に変更 2012/08/27 SETsw kubota ----------------------------------
...
'    '該当データなし
    ...
    sErr_Msg = "2-7 製作条件PGIDチェック"
    If iPGIDCNT = 0 Then
        tbl_chk2_7.sPGIDCHK = "NG"
        funChkFurikae2_7 = 1
        iErr_Code = 2701
        sErr_Msg = "CHECK2-7,製作条件PGIDチェックエラーの為、振り替えできません。"
        gsTbcmy028ErrCode = "02701"
        GoTo Apl_Exit
    End If
...
'          :tOld_Hinban     ,I  ,tFullHinban  :振替元品番(構造体)
'          :tNew_Hinban     ,I  ,tFullHinban  :振替先品番(構造体)
'          :iNew_StartPos   ,I  ,Integer      :振替先品番開始位置
'          :iNew_EndPos     ,I  ,Integer      :振替先品番終了位置
'          :iErr_Code       ,O  ,Integer      :ｴﾗｰｺｰﾄﾞ(戻り値と同一)
'          :sErr_Msg        ,O  ,String       :ｴﾗｰﾒｯｾｰｼﾞｺｰﾄﾞ
'          :戻り値          ,O  ,Integer      :成否(0:正常終了(振替OK),1:正常終了(振替NG),-2:取得ｴﾗｰ)
'説明      :テイル規制チェック
'          :
'履歴      :2012/01/04  DCS)Shoryuji    新規追加
Public Function funChkFurikae2_8(sProccd As String, sBlockId As String, tOld_Hinban As tFullHinban, tNew_Hinban As tFullHinban, _
                                 iNew_StartPos As Integer, iNew_EndPos As Integer, _
                                 iHinCnt As Integer, iErr_Code As Integer, sErr_Msg As String) As Integer
    Dim sql         As String       'SQL全体
    Dim rs          As OraDynaset   'RecordSet
    Dim rsXsdca     As OraDynaset   'RecordSet
    Dim s
    Dim RET         As Integer      '戻り値
    Dim sResult     As String       'コードＤＢ取得関数の取得変数
    Dim i           As Integer
    Dim sBlockNo()  As String           'ブロックID一時保持用
    Dim sBottomRegularNG As String      '振替先ボトム側払出規制範囲
    Dim sTailSpeedManageFLG As String   '振替先テイル育成速度管理フラグ
    Dim sTailPowerManageFLG As String   '振替先テイル開始パワー管理フラグ
    Dim sParentCrystalCode As String    '親結晶番号
    Dim iBlockStartPos As Integer       '構成ブロック開始位置
    Dim iBlockLength As Integer         '構成ブロック長さ
    Dim iPullUpLength As Integer        '引上直胴長さ
    Dim sResultTailSpdMin As String     'テイル速度Min判定結果(総合判定のデータを優先、無ければ結晶引上のデータを保持)
    Dim sResultTailInPower As String    'テイル開始パワー判定結果(総合判定のデータを優先、無ければ結晶引上のデータを保持)
    Dim bTailSpeedCheckResult As Boolean 'テイル速度チェック結果
    Dim bTailPowerCheckResult As Boolean 'テイルパワーチェック結果
    Dim sTspminres As String            'テイル速度Min実績
    Dim sTstpwrres As String            'テイル開始パワー実績
    Dim sTailspmin As String            'テイル速度Min管理値
    Dim sTailinpwrmin As String         'テイル開始パワーMin管理値
    Dim sTailinpwrmax As String         'テイル開始パワーMax管理値
    Dim bJituget As Boolean             '実測値取得フラグ
    ...
    '------------------ 振替先品番のテイル規制設定取得 ------------------------------------------------------
    sErr_Msg = "2-8 品番仕様取得(" & tNew_Hinban.hinban & Format(tNew_Hinban.mnorevno, "00") & tNew_Hinban.factory & tNew_Hinban.opecond & ")"
    
    'SQL文の作成
    sql = vbNullString
    sql = sql & "SELECT TGSPDKFLG, TSPWRKFLG, BTMREGRNG " & vbCrLf
    sql = sql & "FROM   TBCME036 " & vbCrLf
    sql = sql & "WHERE  HINBAN = '" & tNew_Hinban.hinban & "'" & vbCrLf
    sql = sql & "  AND  MNOREVNO = " & tNew_Hinban.mnorevno & vbCrLf
    sql = sql & "  AND  FACTORY = '" & tNew_Hinban.factory & "'" & vbCrLf
    sql = sql & "  AND  OPECOND = '" & tNew_Hinban.opecond & "'"
    ...
    '変数へ設定
    '振替先ボトム側払出規制範囲
    If IsNull(rs("BTMREGRNG")) Then
        sBottomRegularNG = vbNullString
    Else
        sBottomRegularNG = rs("BTMREGRNG")
    End If
    '振替先テイル育成速度管理フラグ
    If IsNull(rs("TGSPDKFLG")) Then
        sTailSpeedManageFLG = vbNullString
    Else
        sTailSpeedManageFLG = rs("TGSPDKFLG")
    End If
    '振替先テイル開始パワー管理フラグ
    If IsNull(rs("TSPWRKFLG")) Then
        sTailPowerManageFLG = vbNullString
    Else
        sTailPowerManageFLG = rs("TSPWRKFLG")
    End If
    ...
    If left(sProccd, 4) = "CW75" Or left(sProccd, 4) = "CW76" Then
        ...
        'シングルIDからブロックIDを取得し、各種情報を取得する
        
        '------------------ 振替元シングルの情報取得 -------------------------------------------------------------
        sErr_Msg = "2-8 シングル情報取得(" & sBlockId & ")"
        sql = vbNullString
        sql = sql & "SELECT CRYNUMCA " & vbCrLf
        sql = sql & "FROM   XSDCA " & vbCrLf
        sql = sql & "WHERE  SXLIDCA = '" & sBlockId & "' AND LIVKCA = '0'"
        ...
        'ブロックID一時保持用の領域再定義
        ReDim sBlockNo(rs.RecordCount)
        
        'ブロックID一時保持用への設定
        For i = 1 To rs.RecordCount
            sBlockNo(i) = rs("CRYNUMCA")
            rs.MoveNext
        Next i
    ...
    '------------------ 振替元ブロックの情報取得 -------------------------------------------------------------
    sErr_Msg = "2-8 ブロック情報取得(" & sBlockNo(1) & ")"
    'SQL文の作成
    sql = vbNullString
    sql = sql & "SELECT XTALC2, INPOSC2, GNLC2 " & vbCrLf
    sql = sql & "FROM   XSDC2 " & vbCrLf
    sql = sql & "WHERE  CRYNUMC2 = '" & sBlockNo(1) & "' AND LIVKC2 = '0'"
    ...
    '変数へ設定
    '親結晶番号
    If IsNull(rs("XTALC2")) Then
        sParentCrystalCode = vbNullString
    Else
        sParentCrystalCode = CStr(rs("XTALC2"))
    End If
    '構成ブロック開始位置
    If IsNull(rs("INPOSC2")) Then
        iBlockStartPos = -1
    Else
        iBlockStartPos = rs("INPOSC2")
    End If
    '構成ブロック長さ
    If IsNull(rs("GNLC2")) Then
        iBlockLength = -1
    Else
        iBlockLength = rs("GNLC2")
    End If
    
    rs.Close
    
    '親結晶番号の内容が無かった場合はブロックIDから結晶番号を生成
    If sParentCrystalCode = vbNullString Then
        sParentCrystalCode = left(sBlockNo, 9) & "000"
    End If
    
    For i = 1 To UBound(sBlockNo)
    
        '------------------ ブロック総合判定測定値情報取得 -------------------------------------------------------
        sErr_Msg = "2-8 総合判定測定値情報取得(" & sBlockNo(i) & ")"
        'SQL文の作成
        sql = vbNullString
        sql = sql & "SELECT TSPMINJUDG, TSTPWRJUDG " & vbCrLf
        sql = sql & "FROM   TBCMJ014 " & vbCrLf
        sql = sql & "WHERE  CRYNUM = '" & sBlockNo(i) & "' " & vbCrLf
        sql = sql & "AND    TBKBN = 'B' " & vbCrLf
        sql = sql & "ORDER BY TRANCNT DESC"
        ...
        If rs.EOF Or rs.RecordCount = 0 Then
            ...
            'テイル速度Min判定結果
            sResultTailSpdMin = vbNullString
            'テイル開始パワー判定結果
            sResultTailInPower = vbNullString
        
        Else
            '該当データ有り
            
            'テイル速度Min判定結果
            If IsNull(rs("TSPMINJUDG")) Then
                
                sResultTailSpdMin = vbNullString
                
            Else
                
                '既に判定OKが設定されていた場合のみ上書き
                If (i = 1) Or (i > 1 And sResultTailSpdMin = "0") Then
                    sResultTailSpdMin = rs("TSPMINJUDG")
                End If
                
            End If
            'テイル開始パワー判定結果
            If IsNull(rs("TSTPWRJUDG")) Then
            ...
            Else
                ...
                '既に判定OKが設定されていた場合のみ上書き
                If (i = 1) Or (i > 1 And sResultTailInPower = "0") Then
                    sResultTailInPower = rs("TSTPWRJUDG")
                End If
    ...
    'CC611(再切断_振替)、CC612(再切断_追込み)の場合は共通変数から判定結果を取得
    If left(sProccd, 4) = "CC61" Then
        
        'テイル速度Min判定結果
        sResultTailSpdMin = tbl_chk2_8.TSPMINJUDGE
        'テイル開始パワー判定結果
        sResultTailInPower = tbl_chk2_8.TSTPWRJUDGE
    
    End If
    
    '判定結果が取得できない場合
'    If sResultTailSpdMin = vbNullString _
'    Or sResultTailInPower = vbNullString Then
'        '異常終了する
'        GoTo Apl_down
'    End If
    
    '------------------ 親結晶仕様情報取得 -------------------------------------------------------------------
    sErr_Msg = "2-8 結晶仕様情報取得(" & sParentCrystalCode & ")"
    'SQL文の作成
    sql = vbNullString
    sql = sql & "SELECT PULENTKC1, TSPMINRESC1, TSPMINJUDGC1, TSTPWRRESC1, TSTPWRJUDGC1 " & vbCrLf
    sql = sql & "FROM   XSDC1 " & vbCrLf
    sql = sql & "WHERE  XTALC1 = '" & sParentCrystalCode & "'"
    ...
    '変数へ設定
    '引上直胴長さ
    If IsNull(rs("PULENTKC1")) Then
        iPullUpLength = -1
    Else
        iPullUpLength = CInt(rs("PULENTKC1"))
    End If
    
'    'テイル速度Min判定結果が取得出来ていなかったら設定する
'    If vbNullString = sResultTailSpdMin Then
'
'        If IsNull(rs("TSPMINJUDGC1")) Then
'            sResultTailSpdMin = vbNullString
'        Else
'            sResultTailSpdMin = rs("TSPMINJUDGC1")
'        End If
'
'    End If
'    'テイル開始パワー判定結果が取得出来ていなかったら設定する
'    If vbNullString = sResultTailInPower Then
'
'        If IsNull(rs("TSTPWRJUDGC1")) Then
'            sResultTailInPower = vbNullString
'        Else
'            sResultTailInPower = rs("TSTPWRJUDGC1")
'        End If
'
'    End If
    ...
    '------------------ テイル規制チェック -------------------------------------------------------------------
    sErr_Msg = "2-8 テイル規制チェック"
    '結果初期化
    bTailSpeedCheckResult = True    'True:振替OK、False:振替NG
    bTailPowerCheckResult = True    'True:振替OK、False:振替NG
    bJituget = False
    
    '振替先テイル育成速度管理フラグが 1 なら
    '振替元のテイル速度規制を判定する
    If sTailSpeedManageFLG = "1" Then
            
        'ボトム側払出規制範囲が設定されている場合に判定する。
        If IsNumeric(sBottomRegularNG) Then
            
            '振替先ボトム側払出規制範囲確認
            '※引上直胴長さ - ボトム側規制範囲 ⇒ テイル規制チェック範囲開始位置
            'テイル規制チェック範囲の開始位置以降に
            '振替先の開始位置もしくは終了位置が含まれる場合にチェック実施。
            If (iPullUpLength - CInt(sBottomRegularNG)) <= iNew_StartPos _
            Or (iPullUpLength - CInt(sBottomRegularNG)) < iNew_EndPos Then
    
                '振替元のテイル速度Min判定結果が"0"(判定OK)の場合、振替OK
                If sResultTailSpdMin = "0" Then
                
                
                '振替元のテイル速度Min判定結果が"1"(判定NG)の場合、振替NG
                ElseIf sResultTailSpdMin = "1" Then
                
                    bTailSpeedCheckResult = False
                
                
                '振替元のテイル速度Min判定結果が判定無しの場合、振替先の実測判定を行う
                Else
                
                    bJituget = True
                    
                    '------------------ XSDC1情報取得 ------------------
                    sErr_Msg = "2-8 XSDC1取得(" & sParentCrystalCode & ")"
                    sql = vbNullString
                    sql = sql & "SELECT TSPMINRESC1 TSPMINRES, " & vbCrLf
                    sql = sql & "       TSTPWRRESC1 TSTPWRRES " & vbCrLf
                    sql = sql & "FROM   XSDC1 " & vbCrLf
                    sql = sql & "WHERE  XTALC1 = '" & sParentCrystalCode & "'"
                    ...
                    '■テイル速度Min実績
                    If IsNull(rs("TSPMINRES")) Then
                        sTspminres = vbNullString
                    Else
                        sTspminres = rs("TSPMINRES")
                    End If
                    '■テイル開始パワー実績
                    If IsNull(rs("TSTPWRRES")) Then
                        sTstpwrres = vbNullString
                    Else
                        sTstpwrres = rs("TSTPWRRES")
                    End If
                    
                    rs.Close
                    
                    '------------------ PG-ID情報取得 ------------------
                    sErr_Msg = "2-8 PG-ID取得(" & sParentCrystalCode & ")"
                    sql = vbNullString
                    sql = sql & "SELECT B011.TAILSPMIN, " & vbCrLf
                    sql = sql & "       B011.TAILPWMIN, " & vbCrLf
                    sql = sql & "       B011.TAILPWMAX " & vbCrLf
                    sql = sql & "FROM   TBCME037 E037, TBCMB011_CCV B011 " & vbCrLf
                    sql = sql & "WHERE  E037.CRYNUM = '" & sParentCrystalCode & "' " & vbCrLf
                    sql = sql & "AND    RTRIM(E037.PGID) = RTRIM(B011.PGID) "
                    ...
                    '■テイル速度Min管理値
                    If IsNull(rs("TAILSPMIN")) Then
                        sTailspmin = vbNullString
                    Else
                        sTailspmin = rs("TAILSPMIN")
                    End If
                    '■テイル開始パワーMin管理値
                    If IsNull(rs("TAILPWMIN")) Then
                        sTailinpwrmin = vbNullString
                    Else
                        sTailinpwrmin = rs("TAILPWMIN")
                    End If
                    '■テイル開始パワーMax管理値
                    If IsNull(rs("TAILPWMAX")) Then
                        sTailinpwrmax = vbNullString
                    Else
                        sTailinpwrmax = rs("TAILPWMAX")
                    End If
                    
                    rs.Close
                    
                    '実績値≧管理値を満たさない場合は判定NG→振替NG
                    If Not (CDbl(sTspminres) >= CDbl(sTailspmin)) Then
                        bTailSpeedCheckResult = False
                    End If
        ...
        Else
            'ボトム側払出規制範囲の設定が取得できない（設定されていない）場合は振替NG
            bTailSpeedCheckResult = False
    ...
    '振替先テイル開始パワー管理フラグが 1 なら
    '振替元のテイル開始パワー規制を判定する
    If sTailPowerManageFLG = "1" Then
        
        'ボトム側払出規制範囲が設定されている場合のみ判定する。
        If IsNumeric(sBottomRegularNG) Then
            ...
            Or (iPullUpLength - CInt(sBottomRegularNG)) < iNew_EndPos Then
                ...
                '振替元のテイル開始パワー判定結果が"0"(判定OK)の場合、振替OK
                If sResultTailInPower = "0" Then
                
                
                '振替元のテイル開始パワー判定結果が"1"(判定NG)の場合、振替NG
                ElseIf sResultTailInPower = "1" Then
                
                    bTailPowerCheckResult = False
                
                
                '振替元のテイル開始パワー判定結果が判定無しの場合、振替先の実測判定を行う
                Else
                
                    '実測値が未取得の場合
                    If Not bJituget Then
                    ...
                    'Max管理値≧実績値≧Min管理値を満たさない場合は判定NG→振替NG
                    If Not (CDbl(sTailinpwrmax) >= CDbl(sTstpwrres) And CDbl(sTstpwrres) >= CDbl(sTailinpwrmin)) Then
                        bTailPowerCheckResult = False
                    End If
        ...
        Else
            'ボトム側払出規制範囲が設定されていない（取得できない）場合は振替NG
            bTailPowerCheckResult = False
    ...
    'テイル規制チェックの振替判定
    If (Not bTailSpeedCheckResult) Or (Not bTailPowerCheckResult) Then
        'テイル速度・テイルパワーどちらかNG判定の場合
        '処理結果に正常終了(振替NG)を設定
        funChkFurikae2_8 = 1
        
        s = IIf(bTailSpeedCheckResult, "1", "0") & IIf(bTailPowerCheckResult, "1", "0")
        
        Select Case s
            Case "10"
            'テイル速度:振替OK、テイルパワー:振替NG　の場合
            iErr_Code = 2801
            'sErr_Msg = "CHECK2-8,テイル開始パワーが管理値外の為、振り替えできません。"
            sErr_Msg = "CHECK2-8,TAIL規制ﾁｪｯｸが判定NGの為、振り替えできません。"
            Case "01"
            'テイル速度:振替NG、テイルパワー:振替OK　の場合
            iErr_Code = 2802
            'sErr_Msg = "CHECK2-8,テイル速度MINが管理値外の為、振り替えできません。"
            sErr_Msg = "CHECK2-8,TAIL規制ﾁｪｯｸが判定NGの為、振り替えできません。"
            Case "00"
            'テイル速度:振替NG、テイルパワー:振替NG　の場合
            iErr_Code = 2803
            'sErr_Msg = "CHECK2-8,テイル速度MIN／テイル開始パワーが管理値外の為、" & vbCrLf & "振り替えできません。"
            sErr_Msg = "CHECK2-8,TAIL規制ﾁｪｯｸが判定NGの為、振り替えできません。"
            Case Else
            '上記以外　の場合
            iErr_Code = 2899
            sErr_Msg = "CHECK2-8,テイル規制チェックエラーの為、振り替えできません。"
        End Select
