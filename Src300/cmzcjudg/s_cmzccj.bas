...
Public Type C_CUDECO
    ...
    CJALLMINDIC5        As Integer      ''CJ 共通Disk半径下限値         2012/05/17 SMPK H.Ohkubo CLESTA評価レベル判定対応
...
''COSF3実績 判定構造体
Public Type C_COSF3
    GuaranteeCOSF3      As Guarantee    ''品質保証情報構造体
    HSXCOSF3PK          As String * 1   ''品ＳＸＣＯＳＦ３パターン区分
    HSXCOSF3SZ          As String * 1   ''品ＳＸＣＯＳＦ３測定条件

    COSF3PTNJSK         As String * 1   ''COSF3 パターン実績
    JudgCOSF3           As Boolean      ''判定結果COSF3
End Type
        ...
        Select Case Res.GuaranteeRes.cPos
          Case "B", "C", "D", "E", "F", "K", "S", "Y"
              ...
              Case "A", "B", "C", "M"
                 ...
                 Res.RRG = MENNAI_Cal_NotRound(RES_JUDG, Res.Res(), Res.GuaranteeRes, Res.GuaranteeRes.cBunp)
...
'概要      :結晶COSF3パターン判定を行う。
'ﾊﾟﾗﾒｰﾀ    :変数名        ,IO ,型               ,説明
'          :Cudeco        ,I  ,COSF3_CUDECO     ,結晶Cu-deco判定構造体
'          :ErrInfo       ,O  ,ERROR_INFOMATION ,エラー情報構造体
'          :戻り値        ,O  ,FUNCTION_RETURN  ,
'説明      :
'履歴      :

'注意）CrystalCOSF3PJudg⇔総合判定（チェック2-1）必ず両方修正すること。

Public Function CrystalCOSF3PJudg(CuDeco As C_COSF3, ErrInfo As ERROR_INFOMATION) As FUNCTION_RETURN
    Dim FuncAns         As FUNCTION_RETURN
    ...
    ''エラー情報構造体初期化
    CrystalCOSF3PJudg = SetErrInfo(ErrInfo)
    
    '*** パターン判定 ***
    bJudg = CudecoJudgPattern(CuDeco.HSXCOSF3PK, CuDeco.COSF3PTNJSK, ptnOK)
    
    CuDeco.JudgCOSF3 = bJudg

    CrystalCOSF3PJudg = FUNCTION_RETURN_SUCCESS
...
'Add Start 2012/05/17 SMPK H.Ohkubo CLESTA評価レベル判定対応
    ...
    If bJudg Then
        '' パターン実績が[Disk] or [Disk & Ring]
        If (CuDeco.CJPTNJSK = CudecoJskPtnD) Or (CuDeco.CJPTNJSK = CudecoJskPtnDR) Then
            ' 共通Disk半径下限値が未入力か150より大きい場合
            If (CuDeco.CJALLMINDIC5 = -1) Or (CuDeco.CJALLMINDIC5 > 150) Then
                bJudg = False
            ' Disk半径実績が未入力か150より大きい場合
            ElseIf (CuDeco.CJDISKJSK = -1) Or (CuDeco.CJDISKJSK > 150) Then
                bJudg = False
            ' 共通Disk半径下限値 > Disk半径実績
            ElseIf (CuDeco.CJALLMINDIC5 > CuDeco.CJDISKJSK) Then
                bJudg = False
            End If
        End If
    End If
        ...
        Select Case Oi.GuaranteeOi.cPos
          Case "B", "C", "D", "E", "F", "K", "Y"
              ...
              Case "A", "B", "C"
                 ...
                 Oi.ORG = MENNAI_Cal_NotRound(OI_JUDG, Oi.Oi(), Oi.GuaranteeOi, Oi.GuaranteeOi.cBunp)
